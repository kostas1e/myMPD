...
#define MATCH(s, n) strcasecmp(section, s) == 0 && strcasecmp(name, n) == 0
    ...
    else if (MATCH("ideon", "nstype"))
    {
        ...
        {
            MYMPD_LOG_WARN("Invalid share type %d", p_config->ns_type);
            p_config->ns_type = 0;
        }
    }
    ...
    {
        MYMPD_LOG_WARN("Unknown config option: %s - %s", section, name);
        return 0;
    }
    return 1;
}
...
static void mympd_get_env(struct t_config *config)
{
    const char *env_vars[] = {"MPD_HOST", "MPD_PORT", "MPD_PASS", "MPD_MUSICDIRECTORY",
                              ...
                              "IDEON_OUTPUTNAME", "IDEON_MIXERTYPE", "IDEON_DOP", "IDEON_NSTYPE", "IDEON_NSSERVER", "IDEON_NSSHARE", "IDEON_SAMBAVERSION",
                              "IDEON_NSUSERNAME", "IDEON_NSPASSWORD", "IDEON_AIRPLAY", "IDEON_ROON", "IDEON_SPOTIFY", "IDEON_INIT", 0};
