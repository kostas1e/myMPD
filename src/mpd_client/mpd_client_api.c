...
void mpd_client_api(t_config *config, t_mpd_client_state *mpd_client_state, void *arg_request)
{
    ...
    case MYMPD_API_SETTINGS_SET:
    {
        ...
        while ((h = json_next_key(request->data, sdslen(request->data), h, ".params", &key, &val)) != NULL)
        {
            ...
            if ((check_mpd_error == true && check_error_and_recover2(mpd_client_state->mpd_state, &notify_buffer, request->method, request->id, true) == false) || rc == false)
            {
        ...
        if (rc == true)
        {
            if (mpd_host_changed == true || mpd_client_state->mpd_state->dc != 0)
            {
