...
void mympd_api_handler(struct t_mympd_state *mympd_state, struct t_partition_state *partition_state, struct t_work_request *request) {
    ...
    // settings
        ...
        case MYMPD_API_SETTINGS_SET: {
            // FIXME
            // old settings
            sds old_mixer_type = sdsnew(mympd_state->mixer_type);
            bool old_dop = mympd_state->dop;
            ...
            if (json_iterate_object(request->data, "$.params", mympd_api_settings_set, mympd_state, NULL, 1000, &parse_error) == true) {
                // FIXME
                // new settings
                bool mpd_conf_changed = (strcmp(old_mixer_type, mympd_state->mixer_type) != 0 || old_dop != mympd_state->dop) ? true : false;
                sds new_ns_settings = sdscatfmt(sdsempty(), "%i%S%S%S%S%S", mympd_state->ns_type, mympd_state->ns_server, mympd_state->ns_share, mympd_state->samba_version, mympd_state->ns_username, mympd_state->ns_password);
            ...
            FREE_SDS(old_mixer_type);
            FREE_SDS(old_ns_settings);
            break;
        }
    ...
    // webradio favorites
        case MYMPD_API_WEBRADIO_FAVORITE_LIST:
            if (json_get_uint(request->data, "$.params.offset", 0, MPD_PLAYLIST_LENGTH_MAX, &uint_buf1, &parse_error) == true &&
                ...
                json_get_string(request->data, "$.params.searchstr", 0, NAME_LEN_MAX, &sds_buf1, vcb_isname, &parse_error) == true)
            {
                response->data = mympd_api_webradio_list(config->workdir, response->data, request->cmd_id, sds_buf1, uint_buf1, uint_buf2);
            }
            break;
