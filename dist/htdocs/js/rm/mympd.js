...
function getCovergridTitleList(id) {
    ...
    if (s) {
        ...
        // console.log('s', s, s.offsetLeft, settings.covergridSize, p, width);
    }
    else {
        width = settings.covergridSize * 2 + 20;
        // console.log('else', settings.covergridSize, width);
    }
...
function appRoute() {
    ...
    if (app.current.app === 'Playback') {
        ...
        // getQueueMini(0);
        // queue card - rm offset
        // getPlaybackQueue(); // current
        // browse card
        getPlaybackBrowse(); // covergrid
    }
...
function appInit() {
    ...
    document.getElementById('searchtidaltags').addEventListener('click', function(event) {
        if (event.target.nodeName === 'BUTTON') {
            app.current.filter = event.target.getAttribute('data-tag');
            searchTidal(domCache.searchtidalstr.value);
        }
    }, false);
...
function setGridPlayback() {
    ...
    if (app.current.app === 'Playback') {
        ...
        // document.getElementById('row1a').classList.add('row');
        // document.getElementById('col1a').classList.add('col-md-12');
        // document.getElementById('cardQueue').classList.remove('hide');
        document.getElementById('cardQueueMini').classList.remove('hide');
        // document.getElementById('cardQueueCurrent').classList.remove('hide');
        // document.getElementById('cardQueueNavCurrent').classList.add('active');
        // document.getElementById('QueueCurrentButtonsTop').classList.add('hide');
        ...
        // document.getElementById('cardHeaderQueue').classList.add('hide');
        ...
        // document.getElementById('row1a').classList.remove('row');
        // document.getElementById('col1a').classList.remove('col-md-12');
        // document.getElementById('QueueCurrentButtonsTop').classList.remove('hide');
...
function getPlaybackQueue() {
    // console.log(settings.colsQueueCurrent);
    let cols = ["Pos", "Title", "Duration"];
    sendAPI("MPD_API_QUEUE_MINI", {"cols": cols}, parseMiniQueue);
    // get all, no pagination, scroll y
    // sendAPI("MPD_API_QUEUE_LIST", {"offset": app.current.page, "cols": settings.colsQueueCurrent}, parseQueue);
}
 */
function getQueueMini(pos) {
    console.log("getminiqueue", pos)
    let cols = ["Pos", "Title", "Artist", "Album", "Duration"];
    sendAPI("MPD_API_QUEUE_MINI", {"pos": pos, "cols": cols}, parseQueueMini);
}

function getPlaybackBrowse() {
    document.getElementById('searchCovergridStr').value = app.current.search;
    selectTag('searchCovergridTags', 'searchCovergridTagsDesc', app.current.filter);
    let sort = app.current.sort;
    let sortdesc = false;
 ...
 */
    ...
    for (let i = 0; i < nrItems; i++) {
        ...
        for (let c = 0; c < cols.length; c++) {
            tds += '<td data-col="' + cols[c] + '">' + e(obj.result.data[i][cols[c]]) + '</td>';
        }
        // tds += '<td data-col="Action"><a href="#" class="material-icons color-darkgrey">' + ligatureMore + '</a></td>';
        row.innerHTML = tds;
        if (i < tr.length) {
...
function showMenuTd(el, event) {
    ...
    if ((app.current.app === 'Browse' && app.current.tab === 'Filesystem') ||
        ...
        // menu += (type === 'song' ? addMenuItem({"cmd": "appendPlayQueue", "options": [type, uri, name]}, t('Append to queue and play')) : '') +
        menu += addMenuItem({"cmd": "appendPlayQueue", "options": [type, uri, name]}, t('Append to queue and play')) +
            addMenuItem({"cmd": "appendQueue", "options": [type, uri, name]}, t('Append to queue')) +
        ...
        if (app.current.app === 'Search') {
            ...
            menu += '<div class="dropdown-divider"></div>' +
                ...
                '<div class="collapse" id="advancedMenu">' +
                    addMenuItem({"cmd": "appendPlayQueue", "options": [type, baseuri, name]}, t('Append to queue and play')) +
        ...
        menu += (type === 'song' ? addMenuItem({"cmd": "appendPlayQueue", "options": [type, uri, name]}, t('Append to queue and play')) : '') +
        (type === 'song' ? addMenuItem({"cmd": "appendQueue", "options": [type, uri, name]}, t('Append to queue')) : '') +
        (type === 'song' ? addMenuItem({"cmd": "appendAfterQueue", "options": [type, uri, nextsongpos, name]}, t('Add after current playing song')) : '') +
        ...
        menu += addMenuItem({"cmd": "getCovergridTitleList", "options": [id]}, t('Show songs')) +
            addMenuItem({"cmd": "addAllFromSearchPlist", "options": ["queue", expression, false, true]}, t('Append to queue and play')) +
...
function setCounter(currentSongId, totalTime, elapsedTime) {
    ...
    if (lastState) {
        if (lastState.currentSongId !== currentSongId) {
            ...
            if (tr) {
                ...
                // tr.removeAttribute('style'); // mv - css
    ...
    if (tr) {
        ...
        tr.style.backgroundColor = '#1d2124'; // mv - css
    }
...
function songChange(obj) {
    ...
    // Update queue (mini) in playback view
    // getQueueMini(obj.result.pos);
}
