                           ...
                           const char *searchstr, const char *filter, const char *sort, bool sortdesc, const unsigned int offset, const unsigned int limit, const char *tag)
{
    ...
    while ((pair = mpd_recv_pair_tag(mpd_client_state->mpd_state->conn, mpdtag)) != NULL)
    {
        ...
        if (entity_count > offset && (entity_count <= offset + limit || limit == 0))
        {
            ...
            else if (searchstr_len == 0 || (searchstr_len <= 2 && strncasecmp(searchstr, pair->value, searchstr_len) == 0) || (searchstr_len > 2 && strcasestr(pair->value, searchstr) != NULL))
            {
                if (entities_returned++)
                {
                    buffer = sdscat(buffer, ",");
...
static bool _cmp_regex(pcre *re_compiled, const char *value)
{
    ...
    if (pcre_exec_ret < 0)
    {
        ...
        case PCRE_ERROR_NULL:
            MYMPD_LOG_ERROR("Something was null");
            break;
        case PCRE_ERROR_BADOPTION:
            MYMPD_LOG_ERROR("A bad option was passed");
            break;
        case PCRE_ERROR_BADMAGIC:
            MYMPD_LOG_ERROR("Magic number bad (compiled regex corrupt?)");
            break;
        case PCRE_ERROR_UNKNOWN_NODE:
            MYMPD_LOG_ERROR("Something kooky in the compiled regex");
            break;
        case PCRE_ERROR_NOMEMORY:
            MYMPD_LOG_ERROR("Ran out of memory");
            break;
        default:
            MYMPD_LOG_ERROR("Unknown error");
            break;
        }
    }
    else
