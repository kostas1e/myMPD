                          ...
                          bool *check_mpd_error)
{
    ...
    *check_mpd_error = false;
    ...
    if (strncmp(key->ptr, "mpdPass", key->len) == 0)
    {
        if (strncmp(val->ptr, "dontsetpassword", val->len) != 0)
        {
            *mpd_host_changed = true;
            mpd_client_state->mpd_state->mpd_pass = sdsreplacelen(mpd_client_state->mpd_state->mpd_pass, settingvalue, sdslen(settingvalue));
        }
        else
        {
    ...
    {
        if (strncmp(val->ptr, mpd_client_state->mpd_state->mpd_host, val->len) != 0)
        {
            *mpd_host_changed = true;
            mpd_client_state->mpd_state->mpd_host = sdsreplacelen(mpd_client_state->mpd_state->mpd_host, settingvalue, sdslen(settingvalue));
        }
    }
    else if (strncmp(key->ptr, "mpdPort", key->len) == 0)
    {
        int mpd_port = strtoimax(settingvalue, &crap, 10);
        if (mpd_client_state->mpd_state->mpd_port != mpd_port)
        {
            *mpd_host_changed = true;
            mpd_client_state->mpd_state->mpd_port = mpd_port;
        }
    }
    ...
    {
        mpd_client_state->music_directory = sdsreplacelen(mpd_client_state->music_directory, settingvalue, sdslen(settingvalue));
    }
    else if (strncmp(key->ptr, "jukeboxMode", key->len) == 0)
    {
        ...
        if (mpd_client_state->jukebox_mode != jukebox_mode)
        {
            mpd_client_state->jukebox_mode = jukebox_mode;
            *jukebox_changed = true;
        }
    }
    else if (strncmp(key->ptr, "jukeboxPlaylist", key->len) == 0)
    {
        if (strcmp(mpd_client_state->jukebox_playlist, settingvalue) != 0)
        {
            mpd_client_state->jukebox_playlist = sdsreplacelen(mpd_client_state->jukebox_playlist, settingvalue, sdslen(settingvalue));
            *jukebox_changed = true;
        }
    }
    else if (strncmp(key->ptr, "jukeboxQueueLength", key->len) == 0)
    {
        ...
        mpd_client_state->jukebox_queue_length = jukebox_queue_length;
    }
    else if (strncmp(key->ptr, "jukeboxLastPlayed", key->len) == 0)
    {
        int jukebox_last_played = strtoimax(settingvalue, &crap, 10);
        if (jukebox_last_played != mpd_client_state->jukebox_last_played)
        {
            mpd_client_state->jukebox_last_played = jukebox_last_played;
            *jukebox_changed = true;
        }
    }
    else if (strncmp(key->ptr, "jukeboxUniqueTag", key->len) == 0)
    {
        ...
        if (mpd_client_state->jukebox_unique_tag.tags[0] != unique_tag)
        {
            mpd_client_state->jukebox_unique_tag.tags[0] = unique_tag;
            *jukebox_changed = true;
        }
    }
    else if (strncmp(key->ptr, "autoPlay", key->len) == 0)
    {
        mpd_client_state->auto_play = val->type == JSON_TYPE_TRUE ? true : false;
    }
    else if (strncmp(key->ptr, "coverimage", key->len) == 0)
    {
        mpd_client_state->coverimage = val->type == JSON_TYPE_TRUE ? true : false;
    }
    else if (strncmp(key->ptr, "coverimageName", key->len) == 0)
    {
        if (validate_string(settingvalue) && sdslen(settingvalue) > 0)
        {
            mpd_client_state->coverimage_name = sdsreplacelen(mpd_client_state->coverimage_name, settingvalue, sdslen(settingvalue));
        }
        else
        {
            sdsfree(settingvalue);
    ...
    {
        mpd_client_state->booklet_name = sdsreplacelen(mpd_client_state->booklet_name, settingvalue, sdslen(settingvalue));
    }
    else if (strncmp(key->ptr, "love", key->len) == 0)
    {
        mpd_client_state->love = val->type == JSON_TYPE_TRUE ? true : false;
    }
    else if (strncmp(key->ptr, "loveChannel", key->len) == 0)
    {
        mpd_client_state->love_channel = sdsreplacelen(mpd_client_state->love_channel, settingvalue, sdslen(settingvalue));
    }
    ...
    {
        mpd_client_state->stickers = val->type == JSON_TYPE_TRUE ? true : false;
    }
    else if (strncmp(key->ptr, "smartpls", key->len) == 0)
    {
        mpd_client_state->smartpls = val->type == JSON_TYPE_TRUE ? true : false;
    }
    else if (strncmp(key->ptr, "smartplsSort", key->len) == 0)
    {
        mpd_client_state->smartpls_sort = sdsreplacelen(mpd_client_state->smartpls_sort, settingvalue, sdslen(settingvalue));
    }
    else if (strncmp(key->ptr, "smartplsPrefix", key->len) == 0)
    {
        mpd_client_state->smartpls_prefix = sdsreplacelen(mpd_client_state->smartpls_prefix, settingvalue, sdslen(settingvalue));
    }
    else if (strncmp(key->ptr, "smartplsInterval", key->len) == 0)
    {
        mpd_client_state->smartpls_interval = strtoumax(settingvalue, &crap, 10);
    }
    ...
    else if (mpd_client_state->mpd_state->conn_state == MPD_CONNECTED)
    {
        ...
        else if (strncmp(key->ptr, "single", key->len) == 0)
        {
            ...
            if (mpd_client_state->feat_single_oneshot == true)
            {
                ...
                {
                    state = MPD_SINGLE_ONESHOT;
                }
                else
                {
                    state = MPD_SINGLE_UNKNOWN;
                }
...
sds mpd_client_put_settings(t_mpd_client_state *mpd_client_state, sds buffer, sds method, long request_id)
{
    ...
    enum mpd_replay_gain_mode replay_gain_mode = mpd_run_replay_gain_status(mpd_client_state->mpd_state->conn);
    ...
    buffer = tojson_bool(buffer, "featNeighbors", mpd_client_state->feat_mpd_neighbor, true);
    mpd_status_free(status);

    buffer = print_tags_array(buffer, "tags", mpd_client_state->mpd_state->mympd_tag_types);
    buffer = sdscat(buffer, ",");
    buffer = print_tags_array(buffer, "searchtags", mpd_client_state->search_tag_types);
    buffer = sdscat(buffer, ",");
