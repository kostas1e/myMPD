...
bool settings_to_webserver(struct t_mympd_state *mympd_state) {
    ...
    // TODO could push dc here as extra
...
bool mympd_api_settings_connection_save(const char *path, sds key, sds value, int vtype, validate_callback vcb, void *userdata, struct t_jsonrpc_parse_error *error) {
    ...
    // TODO remove
    // else if (strcmp(key, "dc") == 0 && vtype == MJSON_TOK_NUMBER) {
    //     int dc = (int)strtoimax(value, NULL, 10);
    //     // if (dc < -1 || dc > 3) {
...
bool mympd_api_settings_set(const char *path, sds key, sds value, int vtype, validate_callback vcb, void *userdata, struct t_jsonrpc_parse_error *error) {
    ...
    // TODO do alphanumeric and bool checks
    else if (strcmp(key, "mixerType") == 0) {
        if (sdscmp(mympd_state->mixer_type, value) != 0) {
            mpd_conf_changed = true;
        }
        mympd_state->mixer_type = sds_replacelen(mympd_state->mixer_type, value, sdslen(value));
    }
    else if (strcmp(key, "dop") == 0) {
        if (vtype == MJSON_TOK_TRUE) {
            if (mympd_state->dop == false) {
                mpd_conf_changed = true;
            }
            mympd_state->dop = true;
        }
        else if (vtype == MJSON_TOK_FALSE) {
            if (mympd_state->dop == true) {
                mpd_conf_changed = true;
            }
    ...
    else if (strcmp(key, "nsType") == 0) {
        // TODO change to unsigned, use enum
        int ns_type = (int)strtoimax(value, NULL, 10);
        if (ns_type < 0 || ns_type > 3) {
            set_invalid_value(error, path, key, value, "Must be a value between 0 and 3");
            return false;
        }
        if (ns_type != mympd_state->ns_type) {
            ns_changed = true;
            mympd_state->ns_type = ns_type;
        }
    }
    else if (strcmp(key, "nsServer") == 0) {
        if (sdscmp(mympd_state->ns_server, value) != 0) {
            ns_changed = true;
        }
        mympd_state->ns_server = sds_replacelen(mympd_state->ns_server, value, sdslen(value));
    }
    else if (strcmp(key, "nsShare") == 0) {
        if (sdscmp(mympd_state->ns_share, value) != 0) {
            ns_changed = true;
        }
        mympd_state->ns_share = sds_replacelen(mympd_state->ns_share, value, sdslen(value));
    }
    else if (strcmp(key, "sambaVersion") == 0) {
        if (sdscmp(mympd_state->samba_version, value) != 0) {
            ns_changed = true;
        }
        mympd_state->samba_version = sds_replacelen(mympd_state->samba_version, value, sdslen(value));
    }
    else if (strcmp(key, "nsUsername") == 0) {
        if (sdscmp(mympd_state->ns_username, value) != 0) {
            ns_changed = true;
        }
        mympd_state->ns_username = sds_replacelen(mympd_state->ns_username, value, sdslen(value));
    }
    else if (strcmp(key, "nsPassword") == 0) {
        if (sdscmp(mympd_state->ns_password, value) != 0) {
            ns_changed = true;
        }
        mympd_state->ns_password = sds_replacelen(mympd_state->ns_password, value, sdslen(value));
    }
    else if (strcmp(key, "airplay") == 0) {
        if (vtype == MJSON_TOK_TRUE) {
            if (mympd_state->airplay == false) {
...
void mympd_api_settings_statefiles_global_read(struct t_mympd_state *mympd_state) {
    ...
    // TODO check vcbs
    mympd_state->mixer_type = state_file_rw_string_sds(workdir, DIR_WORK_STATE, "mixer_type", mympd_state->mixer_type, vcb_isname, true);
    mympd_state->dop = state_file_rw_bool(workdir, DIR_WORK_STATE, "dop", mympd_state->dop, true);
    mympd_state->ns_type = state_file_rw_int(workdir, DIR_WORK_STATE, "ns_type", mympd_state->ns_type, 0, 3, true);
    mympd_state->ns_server = state_file_rw_string_sds(workdir, DIR_WORK_STATE, "ns_server", mympd_state->ns_server, vcb_isname, true);
    mympd_state->ns_share = state_file_rw_string_sds(workdir, DIR_WORK_STATE, "ns_share", mympd_state->ns_share, vcb_isname, true);
    mympd_state->samba_version = state_file_rw_string_sds(workdir, DIR_WORK_STATE, "samba_version", mympd_state->samba_version, vcb_isname, true);
    mympd_state->ns_username = state_file_rw_string_sds(workdir, DIR_WORK_STATE, "ns_username", mympd_state->ns_username, vcb_isname, true);
...
sds mympd_api_settings_get(struct t_partition_state *partition_state, sds buffer, long request_id) {
    ...
    buffer = tojson_char(buffer, "ideonVersion", IDEON_VERSION, true);
