...
void mympd_state_default(struct t_mympd_state *mympd_state, struct t_config *config) {
    ...
    //ideon
    mympd_state->mixer_type = sdsnew("disabled");
    mympd_state->dop = false;
    mympd_state->ns_type = 0;
    mympd_state->ns_server = sdsempty();
    mympd_state->samba_version = sdsnew("vers=3.0");
    mympd_state->ns_share = sdsempty();
    mympd_state->ns_username = sdsempty();
...
void mympd_state_free(struct t_mympd_state *mympd_state) {
    ...
    // FIXME: ideon
    FREE_SDS(mympd_state->mixer_type);
...
void mpd_state_default(struct t_mpd_state *mpd_state, struct t_mympd_state *mympd_state) {
    ...
    // TODO move to partition_state_default
    mpd_state->dc = 0;
