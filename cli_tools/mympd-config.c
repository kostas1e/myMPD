        ...
        if (sdslen(sds_line) > 0)
        {
            ...
            if (tokens == 2)
            {
                if (strcasecmp(name, "music_directory") == 0)
                {
                    ...
                    if (verbose)
                    {
                        printf("\tSetting music directory to %s\n", pconfig->music_directory);
                    }
                }
                else if (strcasecmp(name, "playlist_directory") == 0)
                {
                    ...
                    if (verbose)
                    {
                        printf("\tSetting playlist directory to %s\n", pconfig->playlist_directory);
                    }
                }
...
bool check_ldd(struct t_config *pconfig)
{
    ...
    while (getline(&line, &n, fp) > 0)
    {
        if (strstr(line, "libpcre") != NULL)
        {
            if (verbose)
            {
                printf("\tlibpcre found, enabling regex support\n");
            }
            pconfig->regex = true;
            break;
        }
...
bool parse_options(struct t_config *pconfig, int argc, char **argv)
{
    ...
    while ((n = getopt_long(argc, argv, "c:m:w:s:l:u:e:rpdhytov", long_options, &option_index)) != -1)
    { /* Flawfinder: ignore */
        switch (n)
        {
        case 'c':
