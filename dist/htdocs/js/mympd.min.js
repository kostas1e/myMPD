...
0;a<this.onSettledCallbacks_.length;++a)g.asyncExecute(this.onSettledCallbacks_[a]);this.onSettledCallbacks_=null}};var g=new b;f.prototype.settleSameAsPromise_=function(a){var b=this.createResolveAndReject_();a.callWhenSettled_(b.resolve,b.reject)};f.prototype.settleSameAsThenable_=function(a,b){var c=this.createResolveAndReject_();try{a.call(b,c.resolve,c.reject)}catch(m){c.reject(m)}};f.prototype.then=function(a,b){function c(a,b){return"function"==typeof a?function(b){try{d(a(b))}catch(r){h(r)}}:
...
function(a){navigateTable(this,a.key)},!1);if(c=c.getElementsByTagName("a")[0])c.style.backgroundImage='url("'+subdir+a.result.cover+'"), url("'+subdir+'/assets/coverimage-loading.svg")',c.setAttribute("data-uri",encodeURI(a.result.data[0].uri.replace(/\/[^/]+$/,""))),c.setAttribute("data-name",a.result.Album),c.setAttribute("data-type","dir"),c.addEventListener("click",function(a){showMenu(this,a)},!1);document.getElementById("albumartist"+b).innerText=a.result.AlbumArtist;b="";c=a.result.returnedEntities;
...
domCache.searchCrumb=document.getElementById("searchCrumb");domCache.searchtidalstr=document.getElementById("searchtidalstr");domCache.searchqobuzstr=document.getElementById("searchqobuzstr");
...
document.getElementById("cardSearchDatabase").classList.add("hide");document.getElementById("cardSearchTidal").classList.add("hide");document.getElementById("cardSearchQobuz").classList.add("hide");document.getElementById("card"+app.current.app).classList.remove("hide");document.getElementById("nav"+app.current.app)&&document.getElementById("nav"+app.current.app).classList.add("active");void 0!==app.current.tab&&(document.getElementById("card"+app.current.app+app.current.tab).classList.remove("hide"),
document.getElementById("card"+app.current.app+"Nav"+app.current.tab).classList.add("active"));scrollToPosY(a)}(a=document.getElementById(app.current.app+(void 0===app.current.tab?"":app.current.tab)+(void 0===app.current.view?"":app.current.view)+"List"))&&a.classList.add("opacity05")}
...
a)}else""===domCache.searchstr.value&&""!==app.current.search&&(domCache.searchstr.value=app.current.search);app.last.app!==app.current.app&&""!==app.current.search&&(a=settings["cols"+app.current.app+app.current.tab].length,a--,document.getElementById("SearchDatabaseList").getElementsByTagName("tbody")[0].innerHTML='<tr><td><span class="material-icons">search</span></td><td colspan="'+a+'">'+t("Searching...")+"</td></tr>");2<=domCache.searchstr.value.length||0<domCache.searchCrumb.children.length?
settings.featAdvsearch?(a=app.current.sort,b=!1,"-"===a?(a=settings.tags.includes("Title")?"Title":"-",document.getElementById("SearchDatabaseList").setAttribute("data-sort",a)):0===a.indexOf("-")&&(b=!0,a=a.substring(1)),sendAPI("MPD_API_DATABASE_SEARCH_ADV",{plist:"",offset:app.current.page,sort:a,sortdesc:b,expression:app.current.search,cols:settings.colsSearchDatabase},parseSearch)):sendAPI("MPD_API_DATABASE_SEARCH",{plist:"",offset:app.current.page,filter:app.current.filter,searchstr:app.current.search,
cols:settings.colsSearchDatabase},parseSearch):(document.getElementById("SearchDatabaseList").getElementsByTagName("tbody")[0].innerHTML="",document.getElementById("searchAddAllSongs").setAttribute("disabled","disabled"),document.getElementById("searchAddAllSongsBtn").setAttribute("disabled","disabled"),document.getElementById("cardFooterSearch").innerText="",document.getElementById("SearchDatabaseList").classList.remove("opacity05"),setPagination(0,0));selectTag("searchtags","searchtagsdesc",app.current.filter)}else"Search"===
app.current.app&&"Tidal"===app.current.tab?(domCache.searchtidalstr.focus(),""===domCache.searchtidalstr.value&&""!==app.current.search&&(domCache.searchtidalstr.value=app.current.search),app.last.app!==app.current.app&&""!==app.current.search&&(a=settings["cols"+app.current.app+app.current.tab].length,a--,document.getElementById("SearchTidalList").getElementsByTagName("tbody")[0].innerHTML='<tr><td><span class="material-icons">search</span></td><td colspan="'+a+'">'+t("Searching...")+"</td></tr>"),
2<=domCache.searchtidalstr.value.length?sendAPI("MYMPD_API_TIDAL_SEARCH",{plist:"",offset:app.current.page,filter:app.current.filter,searchstr:app.current.search},parseSearch):(document.getElementById("SearchTidalList").getElementsByTagName("tbody")[0].innerHTML="",document.getElementById("searchTidalAddAllSongs").setAttribute("disabled","disabled"),document.getElementById("searchTidalAddAllSongsBtn").setAttribute("disabled","disabled"),document.getElementById("cardFooterSearch").innerText="",document.getElementById("SearchTidalList").classList.remove("opacity05"),
setPagination(0,0)),selectTag("searchtidaltags","searchtidaltagsdesc",app.current.filter)):"Search"===app.current.app&&"Qobuz"===app.current.tab?(domCache.searchqobuzstr.focus(),""===domCache.searchqobuzstr.value&&""!==app.current.search&&(domCache.searchqobuzstr.value=app.current.search),app.last.app!==app.current.app&&""!==app.current.search&&(a=settings["cols"+app.current.app+app.current.tab].length,a--,document.getElementById("SearchQobuzList").getElementsByTagName("tbody")[0].innerHTML='<tr><td><span class="material-icons">search</span></td><td colspan="'+
a+'">'+t("Searching...")+"</td></tr>"),2<=domCache.searchqobuzstr.value.length?sendAPI("MYMPD_API_QOBUZ_SEARCH",{plist:"",offset:app.current.page,filter:app.current.filter,searchstr:app.current.search},parseSearch):(document.getElementById("SearchQobuzList").getElementsByTagName("tbody")[0].innerHTML="",document.getElementById("searchQobuzAddAllSongs").setAttribute("disabled","disabled"),document.getElementById("searchQobuzAddAllSongsBtn").setAttribute("disabled","disabled"),document.getElementById("cardFooterSearch").innerText=
"",document.getElementById("SearchQobuzList").classList.remove("opacity05"),setPagination(0,0)),selectTag("searchqobuztags","searchqobuztagsdesc",app.current.filter)):appGoto("Playback");app.last.app=app.current.app;app.last.tab=app.current.tab;app.last.view=app.current.view}else appGoto("Playback")}}
...
showAddToPlaylist(app.current.search))},!1);document.getElementById("searchAddAllSongsDropdown").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&("Add all to queue"===a.target.getAttribute("data-phrase")?addAllFromSearchPlist("queue"):"Add all to playlist"===a.target.getAttribute("data-phrase")?showAddToPlaylist("SEARCH"):"Save as smart playlist"===a.target.getAttribute("data-phrase")&&saveSearchAsSmartPlaylist())},!1);document.getElementById("searchTidalAddAllSongsDropdown").addEventListener("click",
...
addAllFromBrowseDatabasePlist("queue"):"Add all to playlist"===a.target.getAttribute("data-phrase")&&showAddToPlaylist("DATABASE"))},!1);document.getElementById("searchtags").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&(app.current.filter=a.target.getAttribute("data-tag"),search(domCache.searchstr.value))},!1);document.getElementById("searchtidaltags").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&(app.current.filter=a.target.getAttribute("data-tag"),searchTidal(domCache.searchtidalstr.value))},
!1);document.getElementById("searchqobuztags").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&(app.current.filter=a.target.getAttribute("data-tag"),searchQobuz(domCache.searchqobuzstr.value))},!1);document.getElementById("searchqueuestr").addEventListener("keyup",function(a){"Escape"===a.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0/"+app.current.filter+"/"+app.current.sort+"/"+this.value)},!1);document.getElementById("searchqueuetags").addEventListener("click",
function(a){"BUTTON"===a.target.nodeName&&appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+"/"+a.target.getAttribute("data-tag")+"/"+app.current.sort+"/"+app.current.search)},!1);a="QueueCurrentColsDropdown BrowseFilesystemColsDropdown SearchDatabaseColsDropdown BrowsePlaylistsDetailColsDropdown BrowseDatabaseColsDropdown PlaybackColsDropdown QueueLastPlayedColsDropdown SearchTidalColsDropdown SearchQobuzColsDropdown".split(" ");for(b=0;b<a.length;b++)document.getElementById(a[b]).addEventListener("click",
function(a){"INPUT"===a.target.nodeName&&a.stopPropagation()},!1);document.getElementById("search").addEventListener("submit",function(){return!1},!1);document.getElementById("searchtidal").addEventListener("submit",function(){return!1},!1);document.getElementById("searchqobuz").addEventListener("submit",function(){return!1},!1);document.getElementById("searchqueue").addEventListener("submit",function(){return!1},!1);domCache.searchstr.addEventListener("keyup",function(a){if("Escape"===a.key)this.blur();
...
b;a.target.remove();search(domCache.searchstr.value)}},!1);domCache.searchtidalstr.addEventListener("keyup",function(a){"Escape"===a.key?this.blur():searchTidal(this.value)},!1);domCache.searchqobuzstr.addEventListener("keyup",function(a){"Escape"===a.key?this.blur():searchQobuz(this.value)},!1);document.getElementById("searchMatch").addEventListener("change",function(){search(domCache.searchstr.value)},!1);document.getElementById("SearchDatabaseList").getElementsByTagName("tr")[0].addEventListener("click",
function(a){if(settings.featAdvsearch&&"TH"===a.target.nodeName){var b=a.target.getAttribute("data-col");if("Duration"!==b){var c=app.current.sort,d=!0;if(c===b||c==="-"+b)0===c.indexOf("-")?(d=!0,b=c.substring(1)):d=!1;!1===d?(c="-"+b,d=!0):(d=!1,c=b);for(var l=document.getElementById("SearchDatabaseList").getElementsByClassName("sort-dir"),k=0;k<l.length;k++)l[k].remove();app.current.sort=c;a.target.innerHTML=t(b)+'<span class="sort-dir material-icons pull-right">'+(!0===d?"arrow_drop_up":"arrow_drop_down")+
"</span>";appGoto(app.current.app,app.current.tab,app.current.view,app.current.page+"/"+app.current.filter+"/"+app.current.sort+"/"+app.current.search)}}},!1);document.getElementById("SearchTidalList").getElementsByTagName("tr")[0].addEventListener("click",function(a){},!1);document.getElementById("SearchQobuzList").getElementsByTagName("tr")[0].addEventListener("click",function(a){},!1);document.getElementById("BrowseDatabaseByTagDropdown").addEventListener("click",function(a){"BUTTON"===a.target.nodeName&&
appGoto(app.current.app,app.current.tab,a.target.getAttribute("data-tag"),"0/"+app.current.filter+"/"+app.current.sort+"/"+app.current.search)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("SearchDatabase");dragAndDropTableHeader("SearchTidal");dragAndDropTableHeader("SearchQobuz");
...
!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(a){navigateTable(this,a.key)},!1);window.addEventListener("beforeinstallprompt",function(a){a.preventDefault();deferredPrompt=a});window.addEventListener("beforeinstallprompt",function(a){a.preventDefault();deferredPrompt=a;domCache.btnAdd.classList.remove("hide")});domCache.btnAdd.addEventListener("click",function(){domCache.btnAdd.classList.add("hide");deferredPrompt.prompt();
...
socket=null);websocketConnected=!1});document.getElementById("localPlayer").addEventListener("canplay",function(){document.getElementById("alertLocalPlayback").classList.add("hide");1==settings.featLocalplayer&&1==settings.localplayerAutoplay&&localplayerPlay()})}appInitStart();function toggleAlert(a,b,c){a=document.getElementById(a);!1===b?(a.innerHTML="",a.classList.add("hide")):(a.innerHTML=c,a.classList.remove("hide"))}
...
return}""!==b?("SEARCH"===a?addAllFromSearchPlist(b):"SEARCHTIDAL"===a?addAllFromSearchTidalPlist(b):"SEARCHQOBUZ"!==a&&("DATABASE"===a?addAllFromBrowseDatabasePlist(b):sendAPI("MPD_API_PLAYLIST_ADD_TRACK",{uri:a,plist:b})),modalAddToPlaylist.hide()):document.getElementById("addToPlaylistPlaylist").classList.add("is-invalid")}
...
"Search"===app.current.app&&"Database"===app.current.tab||"Browse"===app.current.app&&"Database"===app.current.tab?(g+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+("song"===b?addMenuItem({cmd:"appendAfterQueue",options:[b,c,f,d]},t("Add after current playing song")):"")+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+("plist"!==b&&"smartpls"!==b&&settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c]},t("Add to playlist")):"")+("song"===
b?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"")+("plist"===b||"smartpls"===b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):"")+("dir"===b&&settings.featBookmarks?addMenuItem({cmd:"showBookmarkSave",options:[-1,d,c,b]},t("Add bookmark")):""),"Search"===app.current.app&&(c=dirname(c),g+='<div class="dropdown-divider"></div><a class="dropdown-item" id="advancedMenuLink" data-toggle="collapse" href="#advancedMenu"><span class="material-icons material-icons-small-left">keyboard_arrow_right</span>Album actions</a><div class="collapse" id="advancedMenu">'+
addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+addMenuItem({cmd:"appendAfterQueue",options:[b,c,f,d]},t("Add after current playing song"))+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c]},t("Add to playlist")):"")+"</div>")):"Tidal"===app.current.tab?g+=("song"===b?addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Add to queue")):"")+("song"===b?addMenuItem({cmd:"appendAfterQueue",
options:[b,c,f,d]},t("Add after current playing song")):"")+("song"===b?addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue")):"")+("song"===b&&settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c]},t("Add to playlist")):"")+("song"===b?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"")+("album"===b?addMenuItem({cmd:"albumDetails",options:[c]},t("Album details")):"")+("artist"===b?addMenuItem({cmd:"artistDetails",options:[c]},t("Artist info")):"")+("song"===
b?addMenuItem({cmd:"songRadio",options:[c]},t("Go to song radio")):"")+("artist"===b?addMenuItem({cmd:"artistRadio",options:[c]},t("Go to artist radio")):""):"Qobuz"===app.current.tab?g+=("song"!==b?addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Add to queue")):"")+("song"===b?addMenuItem({cmd:"appendAfterQueue",options:[b,c,f,d]},t("Add after current playing song")):"")+("song"===b?addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue")):"")+("song"===b&&settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",
options:[c]},t("Add to playlist")):"")+("song"===b?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"")+("album"===b?addMenuItem({cmd:"albumDetails",options:[c]},t("Album details")):"")+("artist"===b?addMenuItem({cmd:"artistDetails",options:[c]},t("Artist info")):""):"Browse"===app.current.app&&"Playlists"===app.current.tab&&"All"===app.current.view?g+=addMenuItem({cmd:"appendQueue",options:[b,c,d]},t("Append to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,d]},t("Replace queue"))+
("smartpls"===b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):addMenuItem({cmd:"playlistDetails",options:[c]},t("Edit playlist")))+("smartpls"===b?addMenuItem({cmd:"showSmartPlaylist",options:[c]},t("Edit smart playlist")):"")+(!0===settings.smartpls&&"smartpls"===b?addMenuItem({cmd:"updateSmartPlaylist",options:[c]},t("Update smart playlist")):"")+addMenuItem({cmd:"showRenamePlaylist",options:[c]},t("Rename playlist"))+addMenuItem({cmd:"showDelPlaylist",options:[c]},t("Delete playlist")):
...
document.getElementById("search"+b+"AddAllSongsBtn").setAttribute("disabled","disabled"));""===b||"Qobuz"===b?parseFilesystem(a):parseTidal(a)}function saveSearchAsSmartPlaylist(){parseSmartPlaylist({type:"smartpls",result:{playlist:"",type:"search",tag:app.current.filter,searchstr:app.current.search}})}
function addAllFromSearchPlist(a){settings.featAdvsearch?sendAPI("MPD_API_DATABASE_SEARCH_ADV",{plist:a,sort:"",sortdesc:!1,expression:app.current.search,offset:0,cols:settings.colsSearchDatabase}):sendAPI("MPD_API_DATABASE_SEARCH",{plist:a,filter:app.current.filter,searchstr:app.current.search,offset:0,cols:settings.colsSearchDatabase})}
...
settings.streamUrl);toggleBtnChk("btnCoverimage",settings.coverimage);document.getElementById("selectLocale").value=settings.locale;document.getElementById("inputCoverimageName").value=settings.coverimageName;document.getElementById("inputCoverimageSize").value=settings.coverimageSize;document.documentElement.style.setProperty("--mympd-coverimagesize",settings.coverimageSize+"px");document.getElementById("inputBgColor").value=settings.bgColor;document.getElementsByTagName("body")[0].style.backgroundColor=
settings.bgColor;document.getElementById("inputBgCssFilter").value=settings.bgCssFilter;a=document.querySelectorAll(".albumartbg");for(var b=0;b<a.length;b++)a[b].style.filter=settings.bgCssFilter;toggleBtnChk("btnLoveEnable",settings.love);document.getElementById("inputLoveChannel").value=settings.loveChannel;document.getElementById("inputLoveMessage").value=settings.loveMessage;document.getElementById("inputMaxElementsPerPage").value=settings.maxElementsPerPage;toggleBtnChk("btnStickers",settings.stickers);
document.getElementById("inputLastPlayedCount").value=settings.lastPlayedCount;toggleBtnChk("btnSmartpls",settings.smartpls);a=["featLocalplayer","featSyscmds","featMixramp","featCacert","featBookmarks"];for(b=0;b<a.length;b++)for(var c=document.getElementsByClassName(a[b]),d=c.length,f=!0===settings[a[b]]?"":"none",g=0;g<d;g++)c[g].style.display=f;a=document.getElementsByClassName("warnReadonly");for(b=0;b<a.length;b++)!1===settings.readonly?a[b].classList.add("hide"):a[b].classList.remove("hide");
1==settings.readonly?document.getElementById("btnBookmarks").setAttribute("disabled","disabled"):document.getElementById("btnBookmarks").removeAttribute("disabled");if(settings.featSyscmds){a="";b=settings.syscmdList.length;if(0<b)for(a=4<b?"":'<div class="dropdown-divider"></div>',c=0;c<b;c++)a="HR"===settings.syscmdList[c]?a+'<div class="dropdown-divider"></div>':a+('<a class="dropdown-item text-light bg-dark alwaysEnabled" href="#" data-href=\'{"cmd": "execSyscmd", "options": ["'+e(settings.syscmdList[c])+
"\"]}'>"+e(settings.syscmdList[c])+"</a>");document.getElementById("syscmds").innerHTML=a;4<b?(document.getElementById("navSyscmds").classList.remove("hide"),document.getElementById("syscmds").classList.add("collapse","menu-indent")):(document.getElementById("navSyscmds").classList.add("hide"),document.getElementById("syscmds").classList.remove("collapse","menu-indent"))}else document.getElementById("syscmds").innerHTML="";dropdownMainMenu=new Dropdown(document.getElementById("mainMenu"));document.getElementById("selectJukeboxMode").value=
settings.jukeboxMode;document.getElementById("inputJukeboxQueueLength").value=settings.jukeboxQueueLength;0===settings.jukeboxMode?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled")):2===settings.jukeboxMode?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled"),
document.getElementById("selectJukeboxPlaylist").value="Database"):1===settings.jukeboxMode&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"));!0===settings.featLocalplayer&&(""===settings.streamUrl?(settings.mpdstream="http://",null!==settings.mpdHost.match(/^127\./)||"localhost"===settings.mpdHost||null!==settings.mpdHost.match(/^\//)?settings.mpdstream+=window.location.hostname:settings.mpdstream+=
...
"readonly")):"none"===settings.musicDirectory?(document.getElementById("selectMusicDirectory").value=settings.musicDirectory,document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):(document.getElementById("selectMusicDirectory").value="custom",document.getElementById("inputMusicDirectory").value=settings.musicDirectoryValue,document.getElementById("inputMusicDirectory").removeAttribute("readonly"));document.getElementById("inputTidalUser").value=
settings.tidalUsername;document.getElementById("inputTidalPass").value=settings.tidalPassword;document.getElementById("selectTidalQual").value=settings.tidalAudioquality;document.getElementById("inputQobuzUser").value=settings.qobuzUsername;document.getElementById("inputQobuzPass").value=settings.qobuzPassword;document.getElementById("selectQobuzQual").value=settings.qobuzFormatId;"Queue"===app.current.app&&"Current"===app.current.tab?getQueue():"Queue"===app.current.app&&"LastPlayed"===app.current.tab?
appRoute():"Search"===app.current.app?appRoute():"Browse"===app.current.app&&"Filesystem"===app.current.tab?appRoute():"Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view?appRoute():"Browse"===app.current.app&&"Database"===app.current.tab&&""!==app.current.search&&appRoute();i18nHtml(document.getElementsByTagName("body")[0]);checkConsume();settingsParsed="true"}
...
app.apps.Browse.tabs.Database.active="Artist":app.apps.Browse.tabs.Database.active=settings.tags[0]);settings.tags.includes("Title")&&(app.apps.Search.tabs.Database.state="0/any/Title/",app.apps.Search.tabs.Tidal.views.All.state="0/any/Title/",app.apps.Search.tabs.Qobuz.views.All.state="0/any/Title/");settings.featPlaylists?(playlistEl="selectJukeboxPlaylist",sendAPI("MPD_API_PLAYLIST_LIST",{offset:0,filter:"-"},getAllPlaylists)):document.getElementById("selectJukeboxPlaylist").innerHTML='<option value="Database">'+
t("Database")+"</option>";settings.tags.sort();settings.searchtags.sort();settings.searchtidaltags.sort();settings.searchqobuztags.sort();settings.browsetags.sort();filterCols("colsSearchDatabase");filterCols("colsSearchTidal");filterCols("colsSearchQobuz");filterCols("colsQueueCurrent");filterCols("colsQueueLastPlayed");filterCols("colsBrowsePlaylistsDetail");filterCols("colsBrowseFilesystem");filterCols("colsBrowseDatabase");filterCols("colsPlayback");setCols("QueueCurrent");setCols("SearchDatabase");
setCols("SearchTidal");setCols("SearchQobuz");setCols("QueueLastPlayed");setCols("BrowseFilesystem");setCols("BrowsePlaylistsDetail");setCols("BrowseDatabase",".tblAlbumTitles");setCols("Playback");addTagList("BrowseDatabaseByTagDropdown","browsetags");addTagList("searchqueuetags","searchtags");addTagList("searchtags","searchtags");addTagList("searchtidaltags","searchtidaltags");addTagList("searchqobuztags","searchqobuztags");for(a=0;a<settings.tags.length;a++)app.apps.Browse.tabs.Database.views[settings.tags[a]]=
...
validateFloat(c)||(a=!1),c=document.getElementById("inputMixrampdelay"),c.getAttribute("disabled")||("nan"===c.value&&(c.value="-1"),validateFloat(c)||(a=!1)));if(!0===a){a=document.getElementById("selectReplaygain");c=document.getElementById("selectJukeboxPlaylist");f=document.getElementById("selectJukeboxMode");var g=document.getElementById("selectLocale"),h=document.getElementById("selectTidalQual"),l=document.getElementById("selectQobuzQual");sendAPI("MYMPD_API_SETTINGS_SET",{consume:document.getElementById("btnConsume").classList.contains("active")?
...
settings.mixrampdelay,notificationWeb:document.getElementById("btnNotifyWeb").classList.contains("active")?!0:!1,notificationPage:document.getElementById("btnNotifyPage").classList.contains("active")?!0:!1,jukeboxMode:parseInt(f.options[f.selectedIndex].value),jukeboxPlaylist:c.options[c.selectedIndex].value,jukeboxQueueLength:parseInt(document.getElementById("inputJukeboxQueueLength").value),autoPlay:document.getElementById("btnAutoPlay").classList.contains("active")?!0:!1,bgCover:document.getElementById("btnBgCover").classList.contains("active")?
...
coverimageSize:document.getElementById("inputCoverimageSize").value,locale:g.options[g.selectedIndex].value,love:document.getElementById("btnLoveEnable").classList.contains("active")?!0:!1,loveChannel:document.getElementById("inputLoveChannel").value,loveMessage:document.getElementById("inputLoveMessage").value,bookmarks:document.getElementById("btnBookmarks").classList.contains("active")?!0:!1,maxElementsPerPage:document.getElementById("inputMaxElementsPerPage").value,stickers:document.getElementById("btnStickers").classList.contains("active")?
!0:!1,lastPlayedCount:document.getElementById("inputLastPlayedCount").value,smartpls:document.getElementById("btnSmartpls").classList.contains("active")?!0:!1,taglist:getTagMultiSelectValues("listEnabledTags"),searchtaglist:getTagMultiSelectValues("listSearchTags"),browsetaglist:getTagMultiSelectValues("listBrowseTags"),tidalUsername:document.getElementById("inputTidalUser").value,tidalPassword:document.getElementById("inputTidalPass").value,tidalAudioquality:h.options[h.selectedIndex].value,qobuzUsername:document.getElementById("inputQobuzUser").value,
qobuzPassword:document.getElementById("inputQobuzPass").value,qobuzFormatId:parseInt(l.options[l.selectedIndex].value)},getSettings);modalSettings.hide()}}function getTagMultiSelectValues(a){var b=[];a=document.getElementById(a).getElementsByTagName("input");for(var c=0;c<a.length;c++)!0===a[c].checked&&b.push(a[c].name);return b.join(",")}
...
settings[a]=c}function songDetails(a){a.includes("tidal")?sendAPI("MYMPD_API_TIDAL_SONGDETAILS",{uri:a},parseTidalSongDetails):a.includes("qobuz")?sendAPI("MYMPD_API_QOBUZ_SONGDETAILS",{uri:a},parseQobuzSongDetails):sendAPI("MPD_API_DATABASE_SONGDETAILS",{uri:a},parseSongDetails);modalSongDetails.show()}
...
t("Like song")+'" id="btnVoteUp2" data-href=\'{"cmd": "voteSong", "options": [2]}\' class="btn btn-sm btn-light material-icons">thumb_up</button></div></td></tr>');b.getElementsByTagName("tbody")[0].innerHTML=c;setVoteSongBtns(a.result.like,a.result.uri)}
...
"myMPD: "+f;document.getElementById("headerTitle").innerText=f;document.getElementById("headerTitle").title=f;void 0!==a.result.uri?(!0===settings.featStickers&&setVoteSongBtns(a.result.like,a.result.uri),a.result.Filetype=filetype(a.result.uri)):a.result.Filetype="";a.result.Fileformat=lastState?fileformat(lastState.audioFormat):"";for(f=0;f<settings.colsPlayback.length;f++){var g=document.getElementById("current"+settings.colsPlayback[f]);if(g){var h=a.result[settings.colsPlayback[f]];void 0===
...
function setCols(a,b){var c="",d=settings.tags.slice();!1===settings.featTags&&d.push("Title");d.push("Duration");"QueueCurrent"!==a&&"BrowsePlaylistsDetail"!==a&&"QueueLastPlayed"!==a||d.push("Pos");"BrowseFilesystem"===a&&d.push("Type");"QueueLastPlayed"===a&&d.push("LastPlayed");"Playback"===a&&(d.push("Filetype"),d.push("Fileformat"));"SearchTidal"==a&&(d=settings.searchtidaltags.slice(),d.push("Type"),d.push("Duration"),d.push("Track"));d.sort();for(var f=0;f<d.length;f++)if("Playback"!==a||
"Title"!==d[f])c+='<div class="form-check"><input class="form-check-input" type="checkbox" value="1" name="'+d[f]+'"',settings["cols"+a].includes(d[f])&&(c+="checked"),c+='><label class="form-check-label text-light" for="'+d[f]+'">&nbsp;&nbsp;'+t(d[f])+"</label></div>";document.getElementById(a+"ColsDropdown").firstChild.innerHTML=c;d=app.current.sort;"SearchDatabase"===a&&"0/any/Title/"===app.apps.Search.tabs.Database.state&&(d=settings.tags.includes("Title")?"Title":!1===settings.featTags?"Filename":
"-");"SearchTidal"===a&&"0/any/Title/"===app.apps.Search.tabs.Tidal.views.All.state&&(d=settings.tags.includes("Title")?"Title":"-");"SearchQobuz"===a&&"0/any/Title/"===app.apps.Search.tabs.Qobuz.views.All.state&&(d=settings.tags.includes("Title")?"Title":"-");if("Playback"!==a){c="";for(f=0;f<settings["cols"+a].length;f++){var g=settings["cols"+a][f];c+='<th draggable="true" data-col="'+g+'">';if("Track"===g||"Pos"===g)g="#";c+=t(g);"SearchDatabase"!==a&&"SearchTidal"!==a&&"SearchQobuz"!==a||g!==
d&&"-"+g!==d||(g=!1,0===app.current.sort.indexOf("-")&&(g=!0),c+='<span class="sort-dir material-icons pull-right">'+(!0===g?"arrow_drop_up":"arrow_drop_down")+"</span>");c+="</th>"}c+="<th></th>";if(void 0===b)document.getElementById(a+"List").getElementsByTagName("tr")[0].innerHTML=c;else for(a=document.querySelectorAll(b),b=0;b<a.length;b++)a[b].getElementsByTagName("tr")[0].innerHTML=c}}
...
case "M4A":return b+" - MPEG-4 Audio";default:return b}}function fileformat(a){return a.bits+t("bits")+" - "+a.sampleRate/1E3+t("kHz")}function scrollToPosY(a){document.body.scrollTop=a;document.documentElement.scrollTop=a}
...
'<button type="button" class="btn btn-secondary btn-sm btn-block" data-tag="'+settings[b][d]+'">'+t(settings[b][d])+"</button>";document.getElementById(a).innerHTML=c}function openModal(a){window[a].show()}function openDropdown(a){window[a].toggle()}
...
function toggleBtn(a,b){if(a=document.getElementById(a)){if(void 0===b)b=a.classList.contains("active")?!1:!0;else if(0===b||1===b)b=1===b?!0:!1;!0===b?a.classList.add("active"):a.classList.remove("active")}}function toggleBtnChk(a,b){if(a=document.getElementById(a)){if(void 0===b)b=a.classList.contains("active")?!1:!0;else if(0===b||1===b)b=1===b?!0:!1;!0===b?(a.classList.add("active"),a.innerText="check"):(a.classList.remove("active"),a.innerText="radio_button_unchecked")}}
...
function validateStream(a){if(-1<a.value.indexOf("://"))return a.classList.remove("is-invalid"),!0;a.classList.add("is-invalid");return!1}function validateHost(a){if(null!=a.value.match(/^([\w-.]+)$/))return a.classList.remove("is-invalid"),!0;a.classList.add("is-invalid");return!1}function gtTidalPage(a,b,c,d){d=void 0===d?settings.maxElementsPerPage:d;return-1<c?t(a,c):b+app.current.page<d?t(a,b):"> "+t(a,d)}
...
document.getElementById(d+g[h]+"Prev").setAttribute("disabled","disabled")}}function albumDetails(a){a.includes("tidal")?sendAPI("MYMPD_API_TIDAL_ALBUMDETAILS",{uri:a},parseTidalAlbumDetails):a.includes("qobuz")&&sendAPI("MYMPD_API_QOBUZ_ALBUMDETAILS",{uri:a},parseQobuzAlbumDetails);modalAlbumDetails.show()}
...
function artistDetails(a){a.includes("tidal")?sendAPI("MYMPD_API_TIDAL_ARTISTDETAILS",{uri:a},parseTidalArtistDetails):a.includes("qobuz")&&sendAPI("MYMPD_API_QOBUZ_ARTISTDETAILS",{uri:a},parseQobuzArtistDetails);modalArtistDetails.show()}
...
c}function artistRadio(a){a=a.split("://").pop()+"/mix";searchTidal(a)};
