...
function appPrepare(scrollPos) {
    if (app.current.app !== app.last.app || app.current.tab !== app.last.tab || app.current.view !== app.last.view) {
        ...
        setGridPlayback();
        //show active tab
        if (app.current.tab !== undefined && app.current.app !== 'Playback') {
            document.getElementById('tab' + app.current.app + app.current.tab).classList.remove('hide');
        }
        //show active view
        if (app.current.view !== undefined && app.current.app !== 'Playback') {
    ...
    if (list) {
        ...
        if (app.last.app !== 'Playback') {
            document.getElementById('QueueMiniList').classList.add('opacity05');
        }
        document.getElementById('BrowseDatabase' + app.current.view + 'List').classList.add('opacity05');
    }
}
...
function appRoute() {
    ...
    // }
        ...
        sendAPI("MPD_API_DATABASE_FILESYSTEM_LIST", {
            "offset": app.current.offset, "limit": app.current.limit, "path": (app.current.search ? app.current.search : "/"),
        ...
        if (app.current.tag === 'Album') {
            ...
            sendAPI("MPD_API_DATABASE_GET_ALBUMS", {
                "offset": app.current.offset, "limit": app.current.limit, "searchstr": app.current.search,
...
function a2hsInit() {
    ...
    window.addEventListener('appinstalled', function () {
        ...
        showNotification(t('Ideon software installed as app'), '', 'general', 'info');
    });
}
...
function appInitWait() {
    setTimeout(function () {
        ...
        if (settingsParsed === 'true') {
            //parsed settings, now it's safe to connect to websocket
