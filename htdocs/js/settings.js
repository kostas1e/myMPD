...
function parseSettings(obj) {
    ...
    //     clearAndReload();
    // }
    // FIXME reconnect does not parse settings, use welcome message instead
    if ('serviceWorker' in navigator && settings.ideonVersion !== ideonVersion) {
        logWarn('Server version (' + settings.ideonVersion + ') not equal client version (' + ideonVersion + '), reloading');
        clearAndReload();
    }
