...
bool mympd_api_settings_set(const char *path, sds key, sds value, int vtype, validate_callback vcb, void *userdata, struct t_jsonrpc_parse_error *error) {
    ...
    // TODO do alphanumeric and bool checks
    else if (strcmp(key, "mixerType") == 0 && vtype == MJSON_TOK_STRING) {
        if (vcb_isalnum(value) == false) {
            set_invalid_value(error, path, key, value, "Must be avalid mixer type");
            return false;
        }
        ...
        // unsigned ns_type;
        // enum str2int_errno rc = str2uint(&ns_type, value);
        // if (rc != STR2INT_SUCCESS || ns_type > 3) {
        int ns_type = (int)strtoimax(value, NULL, 10);
        if (ns_type < 0 || ns_type > 3) {
            set_invalid_value(error, path, key, value, "Must be a number between 0 and 3");
            return false;
        }
