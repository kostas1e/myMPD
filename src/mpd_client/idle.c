        ...
        bool mpd_idle_event_waiting, struct t_work_request *request)
{
    ...
    switch (partition_state->conn_state) {
        ...
        case MPD_DISCONNECTED:
            // FIXME
            MYMPD_LOG_DEBUG(NULL, "MPD disconnected with dc=%d", partition_state->mpd_state->dc);
            if (partition_state->mpd_state->dc != 0) {
                ideon_dc_handle(&partition_state->mpd_state->dc);
            }
            //try to connect
