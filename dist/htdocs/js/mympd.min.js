...
function getHomeIconPictureList(a){sendAPI("MYMPD_API_HOME_ICON_PICTURE_LIST",{},function(b){for(var c='<option value="">'+t("Use ligature")+"</option>",f=0;f<b.result.returnedEntities;f++)c+='<option value="'+e(b.result.data[f])+'">'+e(b.result.data[f])+"</option>";b=document.getElementById("selectHomeIconImage");b.innerHTML=c;b.value=a})}function checkForUpdates(){sendAPI("MYMPD_API_UPDATE_CHECK",{},parseCheck);btnWaiting(document.getElementById("btnCheckForUpdates"),!0)}
...
checkInit();updateDBstats()}window.onerror=function(a,b,c){logError("JavaScript error: "+a+" ("+b+": "+c+")");4<=settings.loglevel&&(!0===appInited?showNotification(t("JavaScript error"),a+" ("+b+": "+c+")","","danger"):showAppInitAlert(t("JavaScript error")+": "+a+" ("+b+": "+c+")"));return!0};window.onresize=function(){"Playback"===app.current.app&&calcBoxHeight()};appInitStart();
