...
0;logInfo("Connecting to "+a);try{socket.onopen=function(){logInfo("Websocket is connected");websocketConnected=!0;null!==websocketTimer&&(clearTimeout(websocketTimer),websocketTimer=null)},socket.onmessage=function(b){try{var c=JSON.parse(b.data);logDebug("Websocket notification: "+JSON.stringify(c));var f=document.getElementById("currentVersion").innerText;"welcome"===c.method&&""!==f&&c.params.ideonVersion!==f&&clearAndReload()}catch(h){logError("Invalid JSON data received: "+b.data);return}if(c.error)showNotification(t(c.error.message,
c.error.data),"","","danger");else if(c.result)showNotification(t(c.result.message,c.result.data),"","","success");else switch(c.method){case "welcome":websocketConnected=!0;showNotification(t("Connected to Ideon software"),a,"","success");appRoute();sendAPI("MPD_API_PLAYER_STATE",{},parseState,!0);break;case "update_state":c.result=c.params;parseState(c);break;case "mpd_disconnected":progressTimer&&clearTimeout(progressTimer);getSettings(!0);break;case "mpd_connected":showNotification(t("Connected to Streamer"),
"","","success");sendAPI("MPD_API_PLAYER_STATE",{},parseState);getSettings(!0);break;case "update_queue":"Queue"===app.current.app?getQueue():"Playback"===app.current.app&&getQueueMini(c.params.songPos);c.result=c.params;parseUpdateQueue(c);break;case "update_options":getSettings();break;case "update_outputs":sendAPI("MPD_API_PLAYER_OUTPUT_LIST",{},parseOutputs);break;case "update_started":updateDBstarted(!1);break;case "update_database":case "update_finished":updateDBfinished(c.method);updateDBstats();
break;case "update_volume":c.result=c.params;parseVolume(c);break;case "update_stored_playlist":"Browse"===app.current.app&&"Playlists"===app.current.tab&&"All"===app.current.view?sendAPI("MPD_API_PLAYLIST_LIST",{offset:app.current.page,searchstr:app.current.search},parsePlaylists):"Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view&&sendAPI("MPD_API_PLAYLIST_CONTENT_LIST",{offset:app.current.page,searchstr:app.current.search,uri:app.current.filter,cols:settings.colsBrowsePlaylistsDetail},
parsePlaylists);break;case "update_lastplayed":"Queue"===app.current.app&&"LastPlayed"===app.current.tab&&sendAPI("MPD_API_QUEUE_LAST_PLAYED",{offset:app.current.page,cols:settings.colsQueueLastPlayed},parseLastPlayed);break;case "update_jukebox":"Queue"===app.current.app&&"Jukebox"===app.current.tab&&sendAPI("MPD_API_JUKEBOX_LIST",{offset:app.current.page,cols:settings.colsQueueJukebox},parseJukeboxList);break;case "error":document.getElementById("alertMpdState").classList.contains("hide")&&showNotification(t(c.params.message),
"","","danger");break;case "warn":document.getElementById("alertMpdState").classList.contains("hide")&&showNotification(t(c.params.message),"","","warning");break;case "info":document.getElementById("alertMpdState").classList.contains("hide")&&showNotification(t(c.params.message),"","","success")}},socket.onclose=function(){logError("Websocket is disconnected");websocketConnected=!1;!0===appInited?(toggleUI(),progressTimer&&clearTimeout(progressTimer)):(showAppInitAlert(t("Websocket connection failed")),
logError("Websocket connection failed."));null!==websocketTimer&&(clearTimeout(websocketTimer),websocketTimer=null);websocketTimer=setTimeout(function(){logInfo("Reconnecting websocket");toggleAlert("alertMympdState",!0,t("Websocket connection failed, trying to reconnect")+'&nbsp;&nbsp;<div class="spinner-border spinner-border-sm"></div>');webSocketConnect()},3E3);socket=null}}catch(b){logError(b)}}}
...
app.current.sort,a,app.current.search)))}
function gotoBrowse(){if(!1!==settings.featAdvsearch){var a="Playback"===app.current.app?"Playback":"Browse",b=event.target,c=b.getAttribute("data-tag"),f=decodeURI(b.getAttribute("data-name"));null===c&&(c=b.parentNode.getAttribute("data-tag"),f=decodeURI(b.parentNode.getAttribute("data-name")));if(""!==c&&""!==f&&"-"!==f&&settings.browsetags.includes(c))if("Album"===c){var h=b.getAttribute("data-albumartist");null===h&&(h=b.parentNode.getAttribute("data-albumartist"));null!==h?appGoto(a,"Database",
"Detail","0",c,tagAlbumArtist,f,decodeURI(h)):appGoto(a,"Database","List","0",c,tagAlbumArtist,"Album","("+c+" == '"+f+"')")}else appGoto(a,"Database","List","0",c,tagAlbumArtist,"Album","("+c+" == '"+f+"')")}}
...
else{if("Browse"===app.current.app&&"Filesystem"===app.current.tab){var d=document.getElementById("BrowseFilesystemImages");d.innerHTML="";0===a.result.images.length&&""===a.result.bookletPath||!1===settings.publish?d.classList.add("hide"):d.classList.remove("hide");if(""!==a.result.bookletPath&&!0===settings.publish){var g=document.createElement("div");g.style.backgroundImage='url("'+subdir+'/assets/coverimage-booklet.svg")';g.classList.add("booklet");g.setAttribute("data-href",subdir+"/browse/music/"+
a.result.bookletPath);g.title=t("Booklet");d.appendChild(g)}for(g=0;g<a.result.images.length;g++){var k=document.createElement("div");k.style.backgroundImage='url("'+subdir+"/browse/music/"+a.result.images[g]+'"),url("assets/coverimage-loading.svg")';d.appendChild(k)}}d=a.result.returnedEntities;g=f.getElementsByTagName("tr");c=document.activeElement.parentNode.parentNode===c?!0:!1;for(var l=k=0;l<d;l++){var m=encodeURI(a.result.data[l].uri),n=document.createElement("tr"),p="";n.setAttribute("data-type",
a.result.data[l].Type);n.setAttribute("data-uri",m);n.setAttribute("tabindex",0);"Search"===app.current.app&&!0===settings.featTags&&!0===settings.featAdvsearch&&(null!==a.result.data[l].Album&&n.setAttribute("data-album",encodeURI(a.result.data[l].Album)),null!==a.result.data[l][tagAlbumArtist]&&n.setAttribute("data-albumartist",encodeURI(a.result.data[l][tagAlbumArtist])));"song"===a.result.data[l].Type?n.setAttribute("data-name",a.result.data[l].Title):n.setAttribute("data-name",a.result.data[l].name);
switch(a.result.data[l].Type){case "dir":case "smartpls":case "plist":for(m=0;m<settings["cols"+b].length;m++)p+='<td data-col="'+settings["cols"+b][m]+'">',"Type"===settings["cols"+b][m]?p="dir"===a.result.data[l].Type?p+'<span class="material-icons">folder_open</span>':p+('<span class="material-icons">'+("smartpls"===a.result.data[l].Type?"queue_music":"list")+"</span>"):"Title"===settings["cols"+b][m]&&(p+=e(a.result.data[l].name)),p+="</td>";p+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+
ligatureMore+"</a></td>";n.innerHTML=p;break;case "song":void 0!==a.result.data[l].Duration&&(a.result.data[l].Duration=beautifySongDuration(a.result.data[l].Duration));void 0!==a.result.data[l].LastModified&&(a.result.data[l].LastModified=localeDate(a.result.data[l].LastModified));for(m=0;m<settings["cols"+b].length;m++)p+='<td data-col="'+settings["cols"+b][m]+'">',p="Type"===settings["cols"+b][m]?p+'<span class="material-icons">music_note</span>':p+e(a.result.data[l][settings["cols"+b][m]]),p+=
"</td>";p+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a></td>";n.innerHTML=p}l<g.length?k=!0===replaceTblRow(g[l],n)?l:k:f.append(n)}for(b=g.length-1;b>=d;b--)g[b].remove();!0===c&&focusTable(0);setPagination(a.result.totalEntities,a.result.returnedEntities);0===d&&(f.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+h+'">'+t("Empty list")+"</td></tr>");document.getElementById(app.current.app+(void 0===app.current.tab?
"":app.current.tab)+"List").classList.remove("opacity05");scrollToPosY(app.current.scrollPos)}}function addAllFromBrowseFilesystem(){sendAPI("MPD_API_QUEUE_ADD_TRACK",{uri:app.current.search});showNotification(t("Added all songs"),"","","success")}function addAllFromBrowseDatabasePlist(a){2<=app.current.search.length&&sendAPI("MPD_API_DATABASE_SEARCH",{plist:a,filter:app.current.view,searchstr:app.current.search,offset:0,cols:settings.colsSearch,replace:!1})}
...
f.length-1;h>=b;h--)f[h].remove();setPagination(a.result.totalEntities,a.result.returnedEntities);0===b&&(c.innerHTML='<div><span class="material-icons">error_outline</span>&nbsp;'+t("Empty list")+"</div>");"Album"===app.current.tag&&parseDatabaseAlbum(a);"Playback"===app.current.app&&calcBoxHeight();scrollToPosY(app.current.scrollPos)}
...
subdir+"/browse/music/"+e(a.result.bookletPath)+'">'+t("Download booklet")+"</a>")+"</p>";b=document.getElementById("BrowseDatabaseDetailList").getElementsByTagName("tbody")[0];var c=settings.colsBrowseDatabaseDetail.length,f="";1<a.result.Discs&&(f='<tr class="not-clickable"><td><span class="material-icons">album</span></td><td colspan="'+c+'">'+t("Disc 1")+"</td></tr>");for(var h=a.result.returnedEntities,d=parseInt(a.result.data[0].Disc),g=0;g<h;g++){d<parseInt(a.result.data[g].Disc)&&(f+='<tr class="not-clickable"><td><span class="material-icons">album</span></td><td colspan="'+
c+'">'+t("Disc")+" "+e(a.result.data[g].Disc)+"</td></tr>");a.result.data[g].Duration&&(a.result.data[g].Duration=beautifySongDuration(a.result.data[g].Duration));f+='<tr tabindex="0" data-type="song" data-name="'+a.result.data[g].Title+'" data-uri="'+encodeURI(a.result.data[g].uri)+'">';for(d=0;d<settings.colsBrowseDatabaseDetail.length;d++)f+='<td data-col="'+settings.colsBrowseDatabaseDetail[d]+'">'+e(a.result.data[g][settings.colsBrowseDatabaseDetail[d]])+"</td>";f+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+
ligatureMore+"</a></td></tr>";d=a.result.data[g].Disc}b.innerHTML=f;document.getElementById("BrowseDatabaseDetailList").classList.remove("opacity05")}function backToAlbumGrid(){appGoto(app.current.app,"Database","List")}function addAlbum(a){var b=decodeURI(app.current.tag),c=decodeURI(app.current.search);_addAlbum(a,c,b)}
function _addAlbum(a,b,c){b="((Album == '"+c+"') AND ("+tagAlbumArtist+" == '"+b+"'))";"appendPlayQueue"===a?addAllFromSearchPlist("queue",b,!1,!0):"appendQueue"===a?addAllFromSearchPlist("queue",b,!1):"replaceQueue"===a?addAllFromSearchPlist("queue",b,!0,!0):"addPlaylist"===a&&showAddToPlaylist("ALBUM",b)}
function searchAlbumgrid(a){for(var b="",c=document.getElementById("searchDatabaseCrumb").children,f=0;f<c.length;f++)0<f&&(b+=" AND "),b+="("+decodeURI(c[f].getAttribute("data-filter"))+")";""!==a&&(""!==b&&(b+=" AND "),c=document.getElementById("searchDatabaseMatch"),b+="("+app.current.filter+" "+c.options[c.selectedIndex].value+" '"+a+"')");2>=b.length&&(b="");appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,b)}
function navigateGrid(a,b){var c=!1;"Enter"===b?"Browse"!==app.current.app&&"Playback"!==app.current.app||"Database"!==app.current.tab||"List"!==app.current.view?"Home"===app.current.app&&(a=event.target.getAttribute("data-href"),null!==a&&parseCmd(event,a)):("Album"===app.current.tag?appGoto(app.current.app,"Database","Detail","0","Album","AlbumArtist",decodeURI(a.getAttribute("data-album")),decodeURI(a.getAttribute("data-albumartist"))):(app.current.search="",document.getElementById("searchDatabaseStr").value=
"",appGoto(app.current.app,app.current.card,void 0,"0","Album","AlbumArtist","Album","("+app.current.tag+" == '"+decodeURI(a.getAttribute("data-tag"))+"')")),c=!0):" "===b?"Browse"!==app.current.app&&"Playback"!==app.current.app||"Database"!==app.current.tab||"List"!==app.current.view?"Home"===app.current.app&&showMenu(a.getElementsByClassName("card-footer")[0],event):("Album"===app.current.tag&&showMenu(a.getElementsByClassName("card-footer")[0],event),c=!0):"ArrowDown"===b||"ArrowUp"===b?(b="ArrowDown"===
b?null!==a.parentNode.nextElementSibling?a.parentNode.nextElementSibling.firstChild:null:null!==a.parentNode.previousElementSibling?a.parentNode.previousElementSibling.firstChild:null,null!==b&&(b.focus(),a.classList.remove("selected"),b.classList.add("selected"),c=!0,scrollFocusIntoView())):"Escape"===b&&(a.blur(),a.classList.remove("selected"),c=!0);!0===c&&(event.stopPropagation(),event.preventDefault())}
function parseHome(a){var b=a.result.returnedEntities,c=document.getElementById("HomeCards"),f=c.getElementsByClassName("col");0===f.length&&(c.innerHTML="");for(var h=0;h<b;h++){var d=document.createElement("div");d.classList.add("col","px-0","flex-grow-0");""===a.result.data[h].AlbumArtist&&(a.result.data[h].AlbumArtist=t("Unknown artist"));""===a.result.data[h].Album&&(a.result.data[h].Album=t("Unknown album"));var g=JSON.stringify({cmd:a.result.data[h].cmd,options:a.result.data[h].options});g=
'<div class="card home-icons clickable" draggable="true" tabindex="0" data-pos="'+h+"\" data-href='"+e(g)+"'  title=\""+e(a.result.data[h].name)+'"><div class="card-body material-icons">'+e(a.result.data[h].ligature)+'</div><div class="card-footer card-footer-grid p-2">'+e(a.result.data[h].name)+"</div></div>";d.innerHTML=g;h<f.length?f[h].replaceWith(d):c.append(d);""!==a.result.data[h].image&&(d.getElementsByClassName("card-body")[0].style.backgroundImage='url("'+subdir+"/browse/pics/"+a.result.data[h].image+
'")');""!==a.result.data[h].bgcolor&&(d.getElementsByClassName("card-body")[0].style.backgroundColor=a.result.data[h].bgcolor)}for(a=f.length-1;a>=b;a--)f[a].remove();0===b&&(c.innerHTML='<div class="ml-3">'+t("Homescreen welcome")+"</div>")}
...
f.result.data.cmd;showHomeIconCmdOptions(f.result.data.options);getHomeIconPictureList(f.result.data.image);document.getElementById("homeIconPreview").innerText=f.result.data.ligature;document.getElementById("homeIconPreview").style.backgroundColor=f.result.data.bgcolor;""===f.result.data.image?(document.getElementById("divHomeIconLigature").classList.remove("hide"),document.getElementById("homeIconPreview").style.backgroundImage=""):(document.getElementById("divHomeIconLigature").classList.add("hide"),
document.getElementById("homeIconPreview").style.backgroundImage="url("+subdir+'"/browse/pics/'+f.result.data.image+'")');modalEditHomeIcon.show()})}
...
function parseServers(a){var b="";if(a.error)b='<div class="list-group-item"><span class="material-icons">error_outline</span> '+t(a.error.message)+"</div>";else{for(var c=0;c<a.result.returnedEntities;c++)b+='<a href="#" class="list-group-item list-group-item-action" data-value="'+a.result.data[c].server+'">'+a.result.data[c].server+"</a>";0===a.result.returnedEntities&&(b='<div class="list-group-item"><span class="material-icons">error_outline</span>&nbsp;'+t("Empty list")+"</div>")}document.getElementById("dropdownServers").children[0].innerHTML=
b}function checkUpdate(){sendAPI("MYMPD_API_UPDATE_CHECK",{},parseUpdateCheck);btnWaiting(document.getElementById("btnCheckUpdate"),!0)}
function parseUpdateCheck(a){document.getElementById("currentVersion").innerText=a.result.currentVersion;document.getElementById("latestVersion").innerText=a.result.latestVersion;""!==a.result.latestVersion?(!0===a.result.updateAvailable?(document.getElementById("lblInstallUpdate").innerText="New version available",document.getElementById("btnInstallUpdate").classList.remove("hide")):(document.getElementById("lblInstallUpdate").innerText="There is no update available",document.getElementById("btnInstallUpdate").classList.add("hide")),
document.getElementById("updateMsg").innerText=""):(document.getElementById("lblInstallUpdate").innerText="",document.getElementById("btnInstallUpdate").classList.add("hide"),document.getElementById("updateMsg").innerText="Cannot get latest version, please try again later");btnWaiting(document.getElementById("btnCheckUpdate"),!1)}
function installUpdate(){sendAPI("MYMPD_API_UPDATE_INSTALL",{},parseUpdateInstall);document.getElementById("updateMsg").innerText="System will automatically reboot after installation";btnWaiting(document.getElementById("btnInstallUpdate"),!0)}function parseUpdateInstall(a){!1===a.result.service&&(document.getElementById("updateMsg").innerText="Update error, please try again later",btnWaiting(document.getElementById("btnInstallUpdate"),!1))}
...
function saveIdeonSettings(){var a=!0,b=document.getElementById("selectNsType");b=b.options[b.selectedIndex].value;var c=document.getElementById("inputNsServer"),f=document.getElementById("inputNsShare"),h=document.getElementById("inputNsUsername"),d=document.getElementById("inputNsPassword");"0"!==b&&(validateNotBlank(c)||(a=!1),validatePath(f)||(a=!1));"2"===b&&(validateNotBlank(h)&&validateNotBlank(d)||(a=!1));if(!0===a){a=document.getElementById("selectMixerType");var g=document.getElementById("selectSambaVersion");
sendAPI("MYMPD_API_SETTINGS_SET",{mixerType:a.options[a.selectedIndex].value,dop:document.getElementById("btnDop").classList.contains("active")?!0:!1,nsType:parseInt(b),nsServer:c.value,nsShare:f.value,sambaVersion:g.options[g.selectedIndex].value,nsUsername:h.value,nsPassword:d.value,airplay:document.getElementById("btnAirplay").classList.contains("active")?!0:!1,roon:document.getElementById("btnRoon").classList.contains("active")?!0:!1,spotify:document.getElementById("btnSpotify").classList.contains("active")?
!0:!1},getSettings);modalIdeon.hide()}}function confirmReset(){sendAPI("MYMPD_API_SETTINGS_RESET",{},closeReset)}function closeReset(){resetFlag=!1;modalResetSettings.hide()}var currentTab,bgColor;
function checkInit(){!0!==settings.init&&(getServerinfo(),document.getElementById("selectTheme1").innerHTML=document.getElementById("selectTheme").innerHTML,document.getElementById("selectTheme1").value=settings.theme,document.getElementById("selectTheme1").addEventListener("change",function(){var a=getSelectValue(this);"theme-autodetect"===a&&(a=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches?"theme-dark":"theme-default");Object.keys(themes).forEach(function(b){b===a?
domCache.body.classList.add(b):domCache.body.classList.remove(b)});"theme-default"===a?bgColor="#ccc":"theme-light"===a?bgColor="#fff":"theme-dark"===a?bgColor="#000":"theme-maroon"===a&&(bgColor="#800");document.getElementsByTagName("body")[0].style.backgroundColor=bgColor},!1),document.getElementById("selectNsType1").addEventListener("change",function(){var a=this.options[this.selectedIndex].value;"0"===a?(document.getElementById("nsServerShare1").classList.add("hide"),document.getElementById("sambaVersion1").classList.add("hide"),
document.getElementById("nsCredentials1").classList.add("hide"),document.getElementById("inputNsServer1").value="",document.getElementById("inputNsShare1").value="",document.getElementById("inputNsUsername1").value="",document.getElementById("inputNsPassword1").value=""):"2"===a?(document.getElementById("nsServerShare1").classList.remove("hide"),document.getElementById("sambaVersion1").classList.remove("hide"),document.getElementById("nsCredentials1").classList.remove("hide")):(document.getElementById("nsServerShare1").classList.remove("hide"),
"1"===a?document.getElementById("sambaVersion1").classList.remove("hide"):document.getElementById("sambaVersion1").classList.add("hide"),document.getElementById("nsCredentials1").classList.add("hide"),document.getElementById("inputNsUsername1").value="",document.getElementById("inputNsPassword1").value="")},!1),currentTab=0,showTab(currentTab),modalInit.show())}
...
function validateForm(){var a=!0,b=document.getElementById("selectNsType1");b=b.options[b.selectedIndex].value;var c=document.getElementById("inputNsServer1"),f=document.getElementById("inputNsShare1"),h=document.getElementById("inputNsUsername1"),d=document.getElementById("inputNsPassword1");"0"!==b&&(validateIPAddress(c)||(a=!1),validatePath(f)||(a=!1));"2"===b&&(validateNotBlank(h)&&validateNotBlank(d)||(a=!1));return a}
function saveInitSettings(){var a=document.getElementById("selectTheme1"),b=document.getElementById("selectNsType1"),c=document.getElementById("selectSambaVersion1");sendAPI("MYMPD_API_SETTINGS_SET",{theme:a.options[a.selectedIndex].value,bgColor:bgColor,nsType:parseInt(b.options[b.selectedIndex].value),nsServer:document.getElementById("inputNsServer1").value,nsShare:document.getElementById("inputNsShare1").value,sambaVersion:c.options[c.selectedIndex].value,nsUsername:document.getElementById("inputNsUsername1").value,
nsPassword:document.getElementById("inputNsPassword1").value,init:!0},getSettings);modalInit.hide()}function delQueueJukeboxSong(a){sendAPI("MPD_API_JUKEBOX_RM",{pos:a},function(){sendAPI("MPD_API_JUKEBOX_LIST",{offset:app.current.page,cols:settings.colsQueueJukebox},parseJukeboxList)})}
...
c.classList.remove("updateDBprogressAnimate"),c.style.width="100%",c.style.marginLeft="0px",document.getElementById("updateDBfooter").classList.remove("hide"));"update_database"===a?showNotification(t("Database successfully updated"),"","","success"):"update_finished"===a&&showNotification(t("Database update finished"),"","","success")}function updateDBstats(){sendAPI("MPD_API_DATABASE_STATS",{},parseDBstats)}function parseDBstats(a){}
...
var socket=null,websocketConnected=!1,websocketTimer=null,socketRetry=0,lastSong="",lastSongObj={},lastState,currentSong={},playstate="",settingsLock=!1,settingsParsed=!1,settingsNew={},settings={loglevel:2},alertTimeout=null,progressTimer=null,deferredA2HSprompt,dragSrc,dragEl,appInited=!1,subdir="",uiEnabled=!0,locale="en-US",scale="1.0",isMobile=/iPhone|iPad|iPod|Android/i.test(navigator.userAgent),ligatureMore="menu",progressBarTransition="width 1s linear",tagAlbumArtist="AlbumArtist",resetFlag=
!1,app={apps:{Home:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0},Playback:{active:"Database",tabs:{Database:{active:"List",views:{List:{page:0,filter:"AlbumArtist",sort:"AlbumArtist",tag:"Album",search:"",scrollPos:0},Detail:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0}}}}},Queue:{active:"Current",tabs:{Current:{page:0,filter:"any",sort:"-",tag:"-",search:"",scrollPos:0},LastPlayed:{page:0,filter:"any",sort:"-",tag:"-",search:"",scrollPos:0},Jukebox:{page:0,filter:"any",
sort:"-",tag:"-",search:"",scrollPos:0}}},Browse:{active:"Database",tabs:{Filesystem:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0},Playlists:{active:"All",views:{All:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0},Detail:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0}}},Database:{active:"List",views:{List:{page:0,filter:"AlbumArtist",sort:"AlbumArtist",tag:"Album",search:"",scrollPos:0},Detail:{page:0,filter:"-",sort:"-",tag:"-",search:"",scrollPos:0}}}}},Search:{page:0,
filter:"any",sort:"-",tag:"-",search:"",scrollPos:0}},current:{app:"Home",tab:void 0,view:void 0,page:0,filter:"",search:"",sort:"",tag:"",scrollPos:0},last:{app:void 0,tab:void 0,view:void 0,filter:"",search:"",sort:"",tag:"",scrollPos:0}},domCache={};domCache.counter=document.getElementById("counter");domCache.volumePrct=document.getElementById("volumePrct");domCache.volumeControl=document.getElementById("volumeControl");domCache.volumeMenu=document.getElementById("volumeMenu");
...
modalEditHomeIcon=new BSN.Modal(document.getElementById("modalEditHomeIcon")),modalIdeon=new BSN.Modal(document.getElementById("modalIdeon")),modalInit=new BSN.Modal(document.getElementById("modalInit")),modalResetSettings=new BSN.Modal(document.getElementById("modalResetSettings")),dropdownMainMenu=new BSN.Dropdown(document.getElementById("mainMenu")),dropdownVolumeMenu=new BSN.Dropdown(document.getElementById("volumeMenu")),dropdownBookmarks=new BSN.Dropdown(document.getElementById("BrowseFilesystemBookmark")),
dropdownLocalPlayer=new BSN.Dropdown(document.getElementById("localPlaybackMenu")),dropdownPlay=new BSN.Dropdown(document.getElementById("btnPlayDropdown")),dropdownDatabaseSort=new BSN.Dropdown(document.getElementById("btnDatabaseSortDropdown")),dropdownNeighbors=new BSN.Dropdown(document.getElementById("btnDropdownNeighbors")),dropdownServers=new BSN.Dropdown(document.getElementById("btnDropdownServers")),collapseDBupdate=new BSN.Collapse(document.getElementById("navDBupdate")),collapseSettings=
new BSN.Collapse(document.getElementById("navSettings")),collapseSyscmds=new BSN.Collapse(document.getElementById("navSyscmds")),collapseScripting=new BSN.Collapse(document.getElementById("navScripting")),collapseJukeboxMode=new BSN.Collapse(document.getElementById("labelJukeboxMode"));
...
document.getElementById("cardQueueCurrent").classList.add("hide");document.getElementById("cardQueueLastPlayed").classList.add("hide");document.getElementById("cardQueueJukebox").classList.add("hide");document.getElementById("cardBrowsePlaylists").classList.add("hide");document.getElementById("cardBrowseFilesystem").classList.add("hide");document.getElementById("cardBrowseDatabase").classList.add("hide");setGridPlayback();document.getElementById("card"+app.current.app).classList.remove("hide");document.getElementById("nav"+
app.current.app)&&document.getElementById("nav"+app.current.app).classList.add("active");void 0!==app.current.tab&&("Playback"===app.current.app?document.getElementById("cardBrowseDatabase").classList.remove("hide"):document.getElementById("card"+app.current.app+app.current.tab).classList.remove("hide"));scrollToPosY(a)}(a=document.getElementById(app.current.app+(void 0===app.current.tab?"":app.current.tab)+(void 0===app.current.view?"":app.current.view)+"List"))?a.classList.add("opacity05"):"Playback"===
app.current.app&&(document.getElementById("QueueMiniList").classList.add("opacity05"),"Detail"===app.current.view&&document.getElementById("BrowseDatabaseDetailList").classList.add("opacity05"))}
function appGoto(a,b,c,f,h,d,g,k){var l=document.body.scrollTop?document.body.scrollTop:document.documentElement.scrollTop;void 0!==app.apps[app.current.app].scrollPos?app.apps[app.current.app].scrollPos=l:void 0!==app.apps[app.current.app].tabs[app.current.tab].scrollPos?app.apps[app.current.app].tabs[app.current.tab].scrollPos=l:void 0!==app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].scrollPos&&(app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].scrollPos=
l);app.apps[a].tabs?(void 0===b&&(b=app.apps[a].active),app.apps[a].tabs[b].views?(void 0===c&&(c=app.apps[a].tabs[b].active),a="/"+encodeURIComponent(a)+"/"+encodeURIComponent(b)+"/"+encodeURIComponent(c)+"!"+encodeURIComponent(void 0===f?app.apps[a].tabs[b].views[c].page:f)+"/"+encodeURIComponent(void 0===h?app.apps[a].tabs[b].views[c].filter:h)+"/"+encodeURIComponent(void 0===d?app.apps[a].tabs[b].views[c].sort:d)+"/"+encodeURIComponent(void 0===g?app.apps[a].tabs[b].views[c].tag:g)+"/"+encodeURIComponent(void 0===
k?app.apps[a].tabs[b].views[c].search:k)):a="/"+encodeURIComponent(a)+"/"+encodeURIComponent(b)+"!"+encodeURIComponent(void 0===f?app.apps[a].tabs[b].page:f)+"/"+encodeURIComponent(void 0===h?app.apps[a].tabs[b].filter:h)+"/"+encodeURIComponent(void 0===d?app.apps[a].tabs[b].sort:d)+"/"+encodeURIComponent(void 0===g?app.apps[a].tabs[b].tag:g)+"/"+encodeURIComponent(void 0===k?app.apps[a].tabs[b].search:k)):a="/"+encodeURIComponent(a)+"!"+encodeURIComponent(void 0===f?app.apps[a].page:f)+"/"+encodeURIComponent(void 0===
h?app.apps[a].filter:h)+"/"+encodeURIComponent(void 0===d?app.apps[a].sort:d)+"/"+encodeURIComponent(void 0===g?app.apps[a].tag:g)+"/"+encodeURIComponent(void 0===k?app.apps[a].search:k);location.hash=a}
...
app.current.tab,app.apps[app.current.app].tabs[app.current.tab].active=app.current.view,app.current.scrollPos=app.apps[app.current.app].tabs[app.current.tab].views[app.current.view].scrollPos);"Browse"!==app.current.app&&"Playback"!==app.current.app||"Database"!==app.current.tab||setAppState(app.current.page,app.current.filter,app.current.sort,app.current.tag,app.current.search);appPrepare(app.current.scrollPos);if("Home"===app.current.app)sendAPI("MYMPD_API_HOME_LIST",{},parseHome);else if("Queue"===
app.current.app&&"Current"===app.current.tab)selectTag("searchqueuetags","searchqueuetagsdesc",app.current.filter),getQueue();else if("Queue"===app.current.app&&"LastPlayed"===app.current.tab)sendAPI("MPD_API_QUEUE_LAST_PLAYED",{offset:app.current.page,cols:settings.colsQueueLastPlayed},parseLastPlayed);else if("Queue"===app.current.app&&"Jukebox"===app.current.tab)sendAPI("MPD_API_JUKEBOX_LIST",{offset:app.current.page,cols:settings.colsQueueJukebox},parseJukeboxList);else if("Browse"===app.current.app&&
"Playlists"===app.current.tab&&"All"===app.current.view)sendAPI("MPD_API_PLAYLIST_LIST",{offset:app.current.page,searchstr:app.current.search},parsePlaylists),a=document.getElementById("searchPlaylistsStr"),""===a.value&&""!==app.current.search&&(a.value=app.current.search);else if("Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view)sendAPI("MPD_API_PLAYLIST_CONTENT_LIST",{offset:app.current.page,searchstr:app.current.search,uri:app.current.filter,cols:settings.colsBrowsePlaylistsDetail},
parsePlaylists),a=document.getElementById("searchPlaylistsStr"),""===a.value&&""!==app.current.search&&(a.value=app.current.search);else if("Browse"===app.current.app&&"Filesystem"===app.current.tab){sendAPI("MPD_API_DATABASE_FILESYSTEM_LIST",{offset:app.current.page,path:app.current.search?app.current.search:"/",searchstr:"-"!==app.current.filter?app.current.filter:"",cols:settings.colsBrowseFilesystem},parseFilesystem,!0);app.current.search?(document.getElementById("BrowseFilesystemAddAllSongs").removeAttribute("disabled"),
document.getElementById("BrowseFilesystemAddAllSongsBtn").removeAttribute("disabled")):(document.getElementById("BrowseFilesystemAddAllSongs").setAttribute("disabled","disabled"),document.getElementById("BrowseFilesystemAddAllSongsBtn").setAttribute("disabled","disabled"));a='<li class="breadcrumb-item"><a data-uri="" class="text-body material-icons">home</a></li>';for(var b=app.current.search.split("/"),c=b.length,f="",h=0;h<c;h++){if(c-1===h){a+='<li class="breadcrumb-item active">'+e(b[h])+"</li>";
break}f+=b[h];a+='<li class="breadcrumb-item"><a class="text-body" href="#" data-uri="'+encodeURI(f)+'">'+e(b[h])+"</a></li>";f+="/"}document.getElementById("BrowseBreadcrumb").innerHTML=a;a=document.getElementById("searchFilesystemStr");""===a.value&&"-"!==app.current.filter&&(a.value=app.current.filter)}else if("Browse"!==app.current.app&&"Playback"!==app.current.app||"Database"!==app.current.tab||"List"!==app.current.view)if("Browse"!==app.current.app&&"Playback"!==app.current.app||"Database"!==
app.current.tab||"Detail"!==app.current.view)if("Search"===app.current.app){domCache.searchstr.focus();if(settings.featAdvsearch){a="";b=app.current.search.substring(1,app.current.search.length-1).split(" AND ");for(c=0;c<b.length-1;c++)f=b[c].substring(1,b[c].length-1),a+='<button data-filter="'+encodeURI(f)+'" class="btn btn-light mr-2">'+e(f)+'<span class="badge badge-secondary">&times</span></button>';domCache.searchCrumb.innerHTML=a;""===domCache.searchstr.value&&1<=b.length&&(a=b[b.length-1].substring(1,
b[b.length-1].length-1),b=a.substring(a.indexOf("'")+1,a.length-1),domCache.searchstr.value!==b&&(domCache.searchCrumb.innerHTML+='<button data-filter="'+encodeURI(a)+'" class="btn btn-light mr-2">'+e(a)+'<span class="ml-2 badge badge-secondary">&times;</span></button>'),a=a.substring(a.indexOf(" ")+1),a=a.substring(0,a.indexOf(" ")),""===a&&(a="contains"),document.getElementById("searchMatch").value=a)}else""===domCache.searchstr.value&&""!==app.current.search&&(domCache.searchstr.value=app.current.search);
app.last.app!==app.current.app&&""!==app.current.search&&(a=settings["cols"+app.current.app].length,a--,document.getElementById("SearchList").getElementsByTagName("tbody")[0].innerHTML='<tr><td><span class="material-icons">search</span></td><td colspan="'+a+'">'+t("Searching...")+"</td></tr>");2<=domCache.searchstr.value.length||0<domCache.searchCrumb.children.length?settings.featAdvsearch?(a=app.current.sort,b=!1,"-"===a?(a=settings.tags.includes("Title")?"Title":"-",document.getElementById("SearchList").setAttribute("data-sort",
a)):0===a.indexOf("-")&&(b=!0,a=a.substring(1)),sendAPI("MPD_API_DATABASE_SEARCH_ADV",{plist:"",offset:app.current.page,sort:a,sortdesc:b,expression:app.current.search,cols:settings.colsSearch,replace:!1},parseSearch)):sendAPI("MPD_API_DATABASE_SEARCH",{plist:"",offset:app.current.page,filter:app.current.filter,searchstr:app.current.search,cols:settings.colsSearch,replace:!1},parseSearch):(document.getElementById("SearchList").getElementsByTagName("tbody")[0].innerHTML="",document.getElementById("searchAddAllSongs").setAttribute("disabled",
"disabled"),document.getElementById("searchAddAllSongsBtn").setAttribute("disabled","disabled"),document.getElementById("SearchList").classList.remove("opacity05"),setPagination(0,0));selectTag("searchtags","searchtagsdesc",app.current.filter)}else appGoto("Home");else"Playback"===app.current.app&&sendAPI("MPD_API_PLAYER_CURRENT_SONG",{},songChange),document.getElementById("viewListDatabase").classList.add("hide"),document.getElementById("viewDetailDatabase").classList.remove("hide"),"Album"===app.current.filter&&
(a=settings.colsBrowseDatabaseDetail.slice(),!1===a.includes("Disc")&&a.push("Disc"),sendAPI("MPD_API_DATABASE_TAG_ALBUM_TITLE_LIST",{album:app.current.tag,searchstr:app.current.search,tag:app.current.sort,cols:a},parseAlbumDetails));else if("Playback"===app.current.app&&sendAPI("MPD_API_PLAYER_CURRENT_SONG",{},songChange),setDatabaseList(),document.getElementById("viewListDatabase").classList.remove("hide"),document.getElementById("viewDetailDatabase").classList.add("hide"),selectTag("searchDatabaseTags",
"searchDatabaseTagsDesc",app.current.filter),selectTag("BrowseDatabaseByTagDropdown","btnBrowseDatabaseByTagDesc",app.current.tag),a=app.current.sort,b=!1,"-"===app.current.sort.charAt(0)?(b=!0,a=app.current.sort.substr(1),toggleBtnChk("databaseSortDesc",!0)):toggleBtnChk("databaseSortDesc",!1),selectTag("databaseSortTags",void 0,a),"Album"===app.current.tag){c=document.getElementById("searchDatabaseCrumb");f=document.getElementById("searchDatabaseStr");h="";for(var d=app.current.search.split(" AND "),
g=0;g<d.length-1;g++){var k=d[g].substring(1,d[g].length-1);h+='<button data-filter="'+encodeURI(k)+'" class="btn btn-light mr-2">'+e(k)+'<span class="badge badge-secondary">&times</span></button>'}c.innerHTML=h;""===f.value&&1<=d.length&&(h=d[d.length-1].substring(1,d[d.length-1].length-1),d=h.substring(h.indexOf("'")+1,h.length-1),f.value!==d&&(c.innerHTML+='<button data-filter="'+encodeURI(h)+'" class="btn btn-light mr-2">'+e(h)+'<span class="badge badge-secondary">&times;</span></button>'),document.getElementById("searchDatabaseMatch").value=
...
document.getElementById("btnDatabaseSortDropdown").setAttribute("disabled","disabled"),document.getElementById("btnDatabaseSearchDropdown").setAttribute("disabled","disabled"),document.getElementById("searchDatabaseStr").value=app.current.search,sendAPI("MPD_API_DATABASE_TAG_LIST",{offset:app.current.page,searchstr:app.current.search,filter:app.current.filter,sort:a,sortdesc:b,tag:app.current.tag},parseDatabase);app.last.app=app.current.app;app.last.tab=app.current.tab;app.last.view=app.current.view}else appPrepare(0),
!0===settings.featHome?appGoto("Home"):appGoto("Playback")}}function showAppInitAlert(a){document.getElementById("splashScreenAlert").innerHTML='<p class="text-danger">'+t(a)+'</p><p><a id="appReloadBtn" class="btn btn-danger text-light clickable">'+t("Reload")+"</a></p>";document.getElementById("appReloadBtn").addEventListener("click",function(){clearAndReload()},!1)}
function clearAndReload(){"serviceWorker"in navigator&&caches.keys().then(function(a){a.forEach(function(b){caches.delete(b)})});location.reload()}
...
window.addEventListener("appinstalled",function(){logInfo("Ideon software installed as app");showNotification(t("Ideon software installed as app"),"","","success")})}
...
function(d){d.preventDefault();"A"===d.target.nodeName&&(d=d.target.getAttribute("data-value").match(/^(\w+:\/\/)(.+)$/),document.getElementById("selectMountUrlhandler").value=d[1],document.getElementById("inputMountUrl").value=d[2])});document.getElementById("btnDropdownServers").parentNode.addEventListener("show.bs.dropdown",function(){var d=document.getElementById("inputNsWorkgroup").value.toLowerCase();/^\s*$/.test(d)?d='<div class="list-group-item"><span class="material-icons">error_outline</span> '+
t("Workgroup not specified")+"</div>":(sendAPI("MYMPD_API_NS_SERVER_LIST",{workgroup:d},parseServers,!0),d='<div class="list-group-item"><span class="material-icons">search</span> '+t("Searching for servers")+"</div>");document.getElementById("dropdownServers").children[0].innerHTML=d});document.getElementById("dropdownServers").children[0].addEventListener("click",function(d){d.preventDefault();"A"===d.target.nodeName&&(document.getElementById("inputNsServer").value=d.target.getAttribute("data-value"))});
document.getElementById("BrowseFilesystemBookmark").parentNode.addEventListener("show.bs.dropdown",function(){sendAPI("MYMPD_API_BOOKMARK_LIST",{offset:0},parseBookmarks)});document.getElementById("playDropdown").parentNode.addEventListener("show.bs.dropdown",function(){showPlayDropdown()});document.getElementById("playDropdown").addEventListener("click",function(d){d.preventDefault();d.stopPropagation()});a=document.querySelectorAll(".dropdown-toggle");for(b=0;b<a.length;b++)a[b].parentNode.addEventListener("show.bs.dropdown",
function(){alignDropdown(this)});document.getElementById("modalTimer").addEventListener("shown.bs.modal",function(){showListTimer()});document.getElementById("modalMounts").addEventListener("shown.bs.modal",function(){showListMounts()});document.getElementById("modalScripts").addEventListener("shown.bs.modal",function(){showListScripts()});document.getElementById("modalTrigger").addEventListener("shown.bs.modal",function(){showListTrigger()});document.getElementById("modalPartitions").addEventListener("shown.bs.modal",
function(){showListPartitions()});document.getElementById("modalPartitionOutputs").addEventListener("shown.bs.modal",function(){sendAPI("MPD_API_PLAYER_OUTPUT_LIST",{partition:"default"},parsePartitionOutputsList,!1)});document.getElementById("modalAbout").addEventListener("shown.bs.modal",function(){sendAPI("MPD_API_DATABASE_STATS",{},parseStats);getServerinfo();var d="",g;for(g in keymap)if(void 0===keymap[g].req||!0===settings[keymap[g].req])d+='<tr><td><div class="key'+(keymap[g].key&&1<keymap[g].key.length?
" material-icons material-icons-small":"")+'">'+(void 0!==keymap[g].key?keymap[g].key:g)+"</div></td><td>"+t(keymap[g].desc)+"</td></tr>";document.getElementById("tbodyShortcuts").innerHTML=d});document.getElementById("modalAddToPlaylist").addEventListener("shown.bs.modal",function(){document.getElementById("addStreamFrm").classList.contains("hide")?document.getElementById("addToPlaylistPlaylist").focus():(document.getElementById("streamUrl").focus(),document.getElementById("streamUrl").value="")});
document.getElementById("modalAddToPlaylist").addEventListener("keydown",function(d){d.stopPropagation();13===d.keyCode?document.getElementById("addStreamFrm").classList.contains("hide")?addToPlaylist():addStream():27==d.keyCode&&modalAddToPlaylist.hide()});document.getElementById("inputTimerVolume").addEventListener("change",function(){document.getElementById("textTimerVolume").innerHTML=this.value+"&nbsp;%"},!1);document.getElementById("selectTimerAction").addEventListener("change",function(){selectTimerActionChange()},
!1);document.getElementById("selectTriggerScript").addEventListener("change",function(){selectTriggerActionChange()},!1);a="";for(b=0;24>b;b++)a+='<option value="'+b+'">'+zeroPad(b,2)+"</option>";document.getElementById("selectTimerHour").innerHTML=a;a="";for(b=0;60>b;b+=5)a+='<option value="'+b+'">'+zeroPad(b,2)+"</option>";document.getElementById("selectTimerMinute").innerHTML=a;document.getElementById("inputHighlightColor").addEventListener("change",function(){document.getElementById("highlightColorPreview").style.backgroundColor=
this.value},!1);document.getElementById("inputBgColor").addEventListener("change",function(){document.getElementById("bgColorPreview").style.backgroundColor=this.value},!1);document.getElementById("selectTheme").addEventListener("change",function(){var d=getSelectValue(this);"theme-default"===d?document.getElementById("inputBgColor").value="#ccc":"theme-light"===d?document.getElementById("inputBgColor").value="#fff":"theme-dark"===d?document.getElementById("inputBgColor").value="#000":"theme-maroon"===
d&&(document.getElementById("inputBgColor").value="#800");document.getElementById("bgColorPreview").style.backgroundColor=document.getElementById("inputBgColor").value},!1);document.getElementById("modalAddToQueue").addEventListener("shown.bs.modal",function(){document.getElementById("inputAddToQueueQuantity").classList.remove("is-invalid");document.getElementById("warnJukeboxPlaylist2").classList.add("hide");!0===settings.featPlaylists&&sendAPI("MPD_API_PLAYLIST_LIST_ALL",{searchstr:""},function(d){getAllPlaylists(d,
...
resetFlag?getSettings():resetFlag=!1;document.getElementById("inputCrossfade").classList.remove("is-invalid");document.getElementById("inputMixrampdb").classList.remove("is-invalid");document.getElementById("inputMixrampdelay").classList.remove("is-invalid");document.getElementById("inputScaleRatio").classList.remove("is-invalid")});document.getElementById("modalSettings").addEventListener("keydown",function(d){"Enter"===d.key&&(saveSettings(!1),d.stopPropagation(),d.preventDefault())});document.getElementById("modalSettings").addEventListener("hidden.bs.modal",
function(){!0===resetFlag&&modalResetSettings.show()});document.getElementById("modalResetSettings").addEventListener("hidden.bs.modal",function(){modalSettings.show()});document.getElementById("modalIdeon").addEventListener("shown.bs.modal",function(){this.focus();getSettings();document.getElementById("inputNsServer").classList.remove("is-invalid");document.getElementById("inputNsShare").classList.remove("is-invalid");document.getElementById("inputNsUsername").classList.remove("is-invalid");document.getElementById("inputNsPassword").classList.remove("is-invalid")});
document.getElementById("modalIdeon").addEventListener("keydown",function(d){"Enter"===d.key&&(saveIdeonSettings(),d.stopPropagation(),d.preventDefault())});document.getElementById("selectNsType").addEventListener("change",function(){var d=this.options[this.selectedIndex].value;"0"===d?(document.getElementById("nsServerShare").classList.add("hide"),document.getElementById("sambaVersion").classList.add("hide"),document.getElementById("nsCredentials").classList.add("hide"),document.getElementById("inputNsServer").value=
"",document.getElementById("inputNsShare").value="",document.getElementById("inputNsUsername").value="",document.getElementById("inputNsPassword").value=""):"2"===d?(document.getElementById("nsServerShare").classList.remove("hide"),document.getElementById("sambaVersion").classList.remove("hide"),document.getElementById("nsCredentials").classList.remove("hide")):(document.getElementById("nsServerShare").classList.remove("hide"),"1"===d?document.getElementById("sambaVersion").classList.remove("hide"):
document.getElementById("sambaVersion").classList.add("hide"),document.getElementById("nsCredentials").classList.add("hide"),document.getElementById("inputNsUsername").value="",document.getElementById("inputNsPassword").value="")});document.getElementById("modalConnection").addEventListener("shown.bs.modal",function(){getSettings();document.getElementById("inputMpdHost").classList.remove("is-invalid");document.getElementById("inputMpdPort").classList.remove("is-invalid");document.getElementById("inputMpdPass").classList.remove("is-invalid")});
document.getElementById("btnJukeboxModeGroup").addEventListener("mouseup",function(){setTimeout(function(){var d=document.getElementById("btnJukeboxModeGroup").getElementsByClassName("active")[0].getAttribute("data-value");"0"===d?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled")):"2"===d?(document.getElementById("inputJukeboxQueueLength").setAttribute("disabled","disabled"),
document.getElementById("selectJukeboxPlaylist").setAttribute("disabled","disabled"),document.getElementById("selectJukeboxPlaylist").value="Database"):"1"===d&&(document.getElementById("inputJukeboxQueueLength").removeAttribute("disabled"),document.getElementById("selectJukeboxPlaylist").removeAttribute("disabled"));"0"!==d&&toggleBtnChk("btnConsume",!0);checkConsume()},100)});document.getElementById("btnConsume").addEventListener("mouseup",function(){setTimeout(function(){checkConsume()},100)});
document.getElementById("btnStickers").addEventListener("mouseup",function(){setTimeout(function(){document.getElementById("btnStickers").classList.contains("active")?(document.getElementById("warnPlaybackStatistics").classList.add("hide"),document.getElementById("inputJukeboxLastPlayed").removeAttribute("disabled")):(document.getElementById("warnPlaybackStatistics").classList.remove("hide"),document.getElementById("inputJukeboxLastPlayed").setAttribute("disabled","disabled"))},100)});document.getElementById("selectAddToQueueMode").addEventListener("change",
function(){var d=this.options[this.selectedIndex].value;"2"===d?(document.getElementById("inputAddToQueueQuantity").setAttribute("disabled","disabled"),document.getElementById("inputAddToQueueQuantity").value="1",document.getElementById("selectAddToQueuePlaylist").setAttribute("disabled","disabled"),document.getElementById("selectAddToQueuePlaylist").value="Database"):"1"===d&&(document.getElementById("inputAddToQueueQuantity").removeAttribute("disabled"),document.getElementById("selectAddToQueuePlaylist").removeAttribute("disabled"))});
...
settings.musicDirectoryValue,document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):"none"===this.options[this.selectedIndex].value?(document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):(document.getElementById("inputMusicDirectory").value="",document.getElementById("inputMusicDirectory").removeAttribute("readonly"))},!1);document.getElementById("syscmds").addEventListener("click",function(d){"A"===
d.target.nodeName&&parseCmd(d,d.target.getAttribute("data-href"))},!1);document.getElementById("scripts").addEventListener("click",function(d){"A"===d.target.nodeName&&execScript(d.target.getAttribute("data-href"))},!1);a=document.querySelectorAll("[data-href]");b=a.length;for(c=0;c<b;c++){!1===a[c].classList.contains("notclickable")&&a[c].classList.add("clickable");var f=a[c].parentNode.classList.contains("noInitChilds")?!0:!1;!1===f&&(f=a[c].parentNode.parentNode.classList.contains("noInitChilds")?
!0:!1);!0!==f&&a[c].addEventListener("click",function(d){parseCmd(d,this.getAttribute("data-href"))},!1)}document.getElementById("HomeCards").addEventListener("click",function(d){if(d.target.classList.contains("card-body")){var g=d.target.parentNode.getAttribute("data-href");null!==g&&parseCmd(d,g)}else if(d.target.classList.contains("card-footer")){g=document.getElementById("HomeCards").getElementsByClassName("selected");for(var k=0;k<g.length;k++)g[k].classList.remove("selected");d.target.parentNode.classList.add("selected");
showMenu(d.target,d);d.stopPropagation()}},!1);document.getElementById("HomeCards").addEventListener("keydown",function(d){navigateGrid(d.target,d.key)},!1);dragAndDropHome();document.getElementById("selectHomeIconCmd").addEventListener("change",function(){showHomeIconCmdOptions()},!1);document.getElementById("inputHomeIconLigature").addEventListener("change",function(d){document.getElementById("homeIconPreview").innerText=d.target.value;""!==d.target.value&&(document.getElementById("selectHomeIconImage").value=
"",document.getElementById("homeIconPreview").style.backgroundImage="")},!1);document.getElementById("inputHomeIconBgcolor").addEventListener("change",function(d){document.getElementById("homeIconPreview").style.backgroundColor=d.target.value},!1);document.getElementById("selectHomeIconImage").addEventListener("change",function(d){d=getSelectValue(d.target);document.getElementById("homeIconPreview").style.backgroundImage='url("'+subdir+"/browse/pics/"+d+'")';""!==d?(document.getElementById("divHomeIconLigature").classList.add("hide"),
document.getElementById("homeIconPreview").innerHTML=""):(document.getElementById("divHomeIconLigature").classList.remove("hide"),document.getElementById("homeIconPreview").innerText=document.getElementById("inputHomeIconLigature").value)},!1);a=document.getElementsByClassName("pages");b=a.length;for(c=0;c<b;c++)a[c].addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&gotoPage(d.target.getAttribute("data-page"))},!1);document.getElementById("cardPlaybackTags").addEventListener("click",
function(d){"SPAN"===d.target.nodeName&&gotoBrowse()},!1);document.getElementById("BrowseBreadcrumb").addEventListener("click",function(d){"A"===d.target.nodeName&&(d.preventDefault(),appGoto("Browse","Filesystem",void 0,"0",app.current.filter,app.current.sort,"-",decodeURI(d.target.getAttribute("data-uri"))))},!1);document.getElementById("tbodySongDetails").addEventListener("click",function(d){if("A"===d.target.nodeName)if("calcFingerprint"===d.target.id){sendAPI("MPD_API_DATABASE_FINGERPRINT",{uri:decodeURI(d.target.getAttribute("data-uri"))},
parseFingerprint);d.preventDefault();var g=d.target.parentNode,k=document.createElement("div");k.classList.add("spinner-border","spinner-border-sm");d.target.classList.add("hide");g.appendChild(k)}else null!==d.target.parentNode.getAttribute("data-tag")&&(modalSongDetails.hide(),d.preventDefault(),gotoBrowse());else"BUTTON"===d.target.nodeName&&d.target.getAttribute("data-href")&&parseCmd(d,d.target.getAttribute("data-href"))},!1);document.getElementById("outputs").addEventListener("click",function(d){"BUTTON"===
d.target.nodeName?(d.stopPropagation(),d.preventDefault(),sendAPI("MPD_API_PLAYER_TOGGLE_OUTPUT",{output:d.target.getAttribute("data-output-id"),state:d.target.classList.contains("active")?0:1}),toggleBtn(d.target.id)):"A"===d.target.nodeName&&(d.preventDefault(),showListOutputAttributes(decodeURI(d.target.parentNode.getAttribute("data-output-name"))))},!1);document.getElementById("listTimerList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();"TD"===d.target.nodeName?
d.target.parentNode.classList.contains("not-clickable")||showEditTimer(d.target.parentNode.getAttribute("data-id")):"A"===d.target.nodeName?deleteTimer(d.target.parentNode.parentNode.getAttribute("data-id")):"BUTTON"===d.target.nodeName&&toggleTimer(d.target,d.target.parentNode.parentNode.getAttribute("data-id"))},!1);document.getElementById("listMountsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("TD"===d.target.nodeName){if(""===d.target.parentNode.getAttribute("data-point"))return!1;
showEditMount(decodeURI(d.target.parentNode.getAttribute("data-url")),decodeURI(d.target.parentNode.getAttribute("data-point")))}else if("A"===d.target.nodeName){var g=d.target.getAttribute("data-action"),k=decodeURI(d.target.parentNode.parentNode.getAttribute("data-point"));"unmount"===g?unmountMount(k):"update"===g&&updateMount(d.target,k)}},!1);document.getElementById("listScriptsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("TD"===d.target.nodeName){if(!1===
settings.featScripteditor||""===d.target.parentNode.getAttribute("data-script"))return!1;showEditScript(decodeURI(d.target.parentNode.getAttribute("data-script")))}else if("A"===d.target.nodeName){var g=d.target.getAttribute("data-action"),k=decodeURI(d.target.parentNode.parentNode.getAttribute("data-script"));"delete"===g?deleteScript(k):"execute"===g?execScript(d.target.getAttribute("data-href")):"add2home"===g&&addScriptToHome(k,d.target.getAttribute("data-href"))}},!1);document.getElementById("listTriggerList").addEventListener("click",
function(d){d.stopPropagation();d.preventDefault();if("TD"===d.target.nodeName){var g=decodeURI(d.target.parentNode.getAttribute("data-trigger-id"));showEditTrigger(g)}else"A"===d.target.nodeName&&(g=d.target.getAttribute("data-action"),d=decodeURI(d.target.parentNode.parentNode.getAttribute("data-trigger-id")),"delete"===g&&deleteTrigger(d))},!1);document.getElementById("listPartitionsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();if("A"===d.target.nodeName){var g=
d.target.getAttribute("data-action");d=decodeURI(d.target.parentNode.parentNode.getAttribute("data-partition"));"delete"===g?deletePartition(d):"switch"===g&&switchPartition(d)}},!1);document.getElementById("partitionOutputsList").addEventListener("click",function(d){d.stopPropagation();d.preventDefault();"TD"===d.target.nodeName&&(d=decodeURI(d.target.parentNode.getAttribute("data-output")),moveOutput(d),modalPartitionOutputs.hide())},!1);document.getElementById("QueueCurrentList").addEventListener("click",
function(d){"TD"===d.target.nodeName?sendAPI("MPD_API_PLAYER_PLAY_TRACK",{track:d.target.parentNode.getAttribute("data-trackid")}):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("QueueLastPlayedList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendPlayQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("QueueJukeboxList").addEventListener("click",
function(d){"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("QueueMiniList").addEventListener("click",function(d){"TD"===d.target.nodeName&&sendAPI("MPD_API_PLAYER_PLAY_TRACK",{track:d.target.parentNode.getAttribute("data-trackid")})},!1);document.getElementById("BrowseFilesystemList").addEventListener("click",function(d){if("TD"===d.target.nodeName)switch(d.target.parentNode.getAttribute("data-type")){case "parentDir":case "dir":appGoto("Browse","Filesystem",void 0,"0",
app.current.filter,app.current.sort,"-",decodeURI(d.target.parentNode.getAttribute("data-uri")));break;case "song":appendPlayQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name"));break;case "plist":case "smartpls":playlistDetails(decodeURI(d.target.parentNode.getAttribute("data-uri")))}else"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseFilesystemBookmarks").addEventListener("click",function(d){if("A"===
d.target.nodeName){var g=d.target.parentNode.parentNode.getAttribute("data-id"),k=d.target.parentNode.parentNode.getAttribute("data-type"),l=decodeURI(d.target.parentNode.parentNode.getAttribute("data-uri")),m=d.target.parentNode.parentNode.firstChild.innerText,n=d.target.getAttribute("data-href");"delete"===n?(sendAPI("MYMPD_API_BOOKMARK_RM",{id:g},function(){sendAPI("MYMPD_API_BOOKMARK_LIST",{offset:0},parseBookmarks)}),d.preventDefault(),d.stopPropagation()):"edit"===n?showBookmarkSave(g,m,l,k):
"goto"===n&&appGoto("Browse","Filesystem",void 0,"0","-","-","-",l)}},!1);document.getElementById("BrowsePlaylistsAllList").addEventListener("click",function(d){"TD"===d.target.nodeName?playlistDetails(decodeURI(d.target.parentNode.getAttribute("data-uri"))):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowsePlaylistsDetailList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendPlayQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),
d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("SearchList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendPlayQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseDatabaseCards").addEventListener("click",function(d){"Album"===app.current.tag?d.target.classList.contains("card-body")?
appGoto(app.current.app,"Database","Detail","0","Album","AlbumArtist",decodeURI(d.target.parentNode.getAttribute("data-album")),decodeURI(d.target.parentNode.getAttribute("data-albumartist"))):d.target.classList.contains("card-footer")&&showMenu(d.target,d):(app.current.search="",document.getElementById("searchDatabaseStr").value="",appGoto(app.current.app,app.current.card,void 0,"0","Album","AlbumArtist","Album","("+app.current.tag+" == '"+decodeURI(d.target.parentNode.getAttribute("data-tag"))+
"')"))},!1);document.getElementById("BrowseDatabaseAlbumList").addEventListener("click",function(d){"TD"===d.target.nodeName?appGoto(app.current.app,"Database","Detail","0","Album","AlbumArtist",decodeURI(d.target.parentNode.getAttribute("data-album")),decodeURI(d.target.parentNode.getAttribute("data-albumartist"))):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseDatabaseCards").addEventListener("keydown",function(d){navigateGrid(d.target,d.key)},!1);!1===isMobile&&
(document.getElementById("BrowseDatabaseCards").addEventListener("mouseover",function(d){if(d.target.classList.contains("card-body")&&0===d.target.childNodes.length){var g=document.getElementById("BrowseDatabaseCards").getElementsByClassName("album-grid-mouseover");if(1<g.length)for(var k=0;k<g.length;k++)g[k].remove();addPlayButton(d.target)}},!1),document.getElementById("BrowseDatabaseCards").addEventListener("mouseout",function(d){!d.target.classList.contains("card-body")||null!==d.relatedTarget&&
d.relatedTarget.classList.contains("album-grid-mouseover")||(d.target.innerHTML="")},!1));document.getElementById("BrowseDatabaseDetailList").addEventListener("click",function(d){"TD"===d.target.nodeName?appendPlayQueue("song",decodeURI(d.target.parentNode.getAttribute("data-uri")),d.target.parentNode.getAttribute("data-name")):"A"===d.target.nodeName&&showMenu(d.target,d)},!1);document.getElementById("BrowseFilesystemAddAllSongsDropdown").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&
("Add all to queue"===d.target.getAttribute("data-phrase")?addAllFromBrowseFilesystem():"Add all to playlist"===d.target.getAttribute("data-phrase")&&showAddToPlaylist(app.current.search,""))},!1);document.getElementById("searchAddAllSongsDropdown").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&("Add all to queue"===d.target.getAttribute("data-phrase")?addAllFromSearchPlist("queue",null,!1):"Add all to playlist"===d.target.getAttribute("data-phrase")?showAddToPlaylist("SEARCH",
""):"Save as smart playlist"===d.target.getAttribute("data-phrase")&&saveSearchAsSmartPlaylist())},!1);document.getElementById("searchtags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(app.current.filter=d.target.getAttribute("data-tag"),doSearch(domCache.searchstr.value))},!1);document.getElementById("searchDatabaseTags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(app.current.filter=d.target.getAttribute("data-tag"),appGoto(app.current.app,app.current.tab,
app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,app.current.search))},!1);document.getElementById("databaseSortDesc").addEventListener("click",function(d){toggleBtnChk(this);d.stopPropagation();d.preventDefault();"-"===app.current.sort.charAt(0)?app.current.sort=app.current.sort.substr(1):app.current.sort="-"+app.current.sort;appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,app.current.search)},!1);document.getElementById("databaseSortTags").addEventListener("click",
function(d){"BUTTON"===d.target.nodeName&&(d.preventDefault(),d.stopPropagation(),app.current.sort=d.target.getAttribute("data-tag"),appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,app.current.search))},!1);document.getElementById("BrowseDatabaseByTagDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("BrowseNavPlaylistsDropdown").addEventListener("click",function(d){navBrowseHandler(d)},
!1);document.getElementById("BrowseNavFilesystemDropdown").addEventListener("click",function(d){navBrowseHandler(d)},!1);document.getElementById("dropdownSortPlaylistTags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&(d.preventDefault(),playlistSort(d.target.getAttribute("data-tag")))},!1);document.getElementById("searchqueuestr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,
app.current.sort,"-",this.value)},!1);document.getElementById("searchqueuetags").addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&appGoto(app.current.app,app.current.tab,app.current.view,app.current.page,d.target.getAttribute("data-tag"),app.current.sort,"-",app.current.search)},!1);document.getElementById("searchFilesystemStr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",""!==this.value?this.value:
"-",app.current.sort,"-",app.current.search)},!1);document.getElementById("searchPlaylistsStr").addEventListener("keyup",function(d){"Escape"===d.key?this.blur():appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,"-",this.value)},!1);a=["PlaybackColsDropdown"];for(b=0;b<a.length;b++)document.getElementById(a[b]).addEventListener("click",function(d){"BUTTON"===d.target.nodeName&&d.target.classList.contains("material-icons")&&(d.stopPropagation(),d.preventDefault(),
toggleBtnChk(d.target))},!1);document.getElementById("search").addEventListener("submit",function(){return!1},!1);document.getElementById("searchqueue").addEventListener("submit",function(){return!1},!1);document.getElementById("searchdatabase").addEventListener("submit",function(){return!1},!1);document.getElementById("searchDatabaseStr").addEventListener("keyup",function(d){if("Escape"===d.key)this.blur();else if("Enter"===d.key&&"Album"===app.current.tag)if(""!==this.value){d=document.getElementById("searchDatabaseMatch");
var g=document.createElement("button");g.classList.add("btn","btn-light","mr-2");g.setAttribute("data-filter",encodeURI(app.current.filter+" "+d.options[d.selectedIndex].value+" '"+this.value+"'"));g.innerHTML=e(app.current.filter)+" "+e(d.options[d.selectedIndex].value)+" '"+e(this.value)+'\'<span class="ml-2 badge badge-secondary">&times;</span>';this.value="";document.getElementById("searchDatabaseCrumb").appendChild(g)}else searchAlbumgrid(this.value);else"Album"===app.current.tag?searchAlbumgrid(this.value):
appGoto(app.current.app,app.current.tab,app.current.view,"0",app.current.filter,app.current.sort,app.current.tag,this.value)},!1);document.getElementById("searchDatabaseCrumb").addEventListener("click",function(d){d.preventDefault();d.stopPropagation();if("SPAN"===d.target.nodeName)d.target.parentNode.remove(),searchAlbumgrid("");else if("BUTTON"===d.target.nodeName){var g=decodeURI(d.target.getAttribute("data-filter"));document.getElementById("searchDatabaseStr").value=g.substring(g.indexOf("'")+
1,g.length-1);var k=g.substring(g.indexOf(" ")+1);k=k.substring(0,k.indexOf(" "));document.getElementById("searchDatabaseMatch").value=k;g=g.substring(0,g.indexOf(" "));selectTag("searchDatabaseTags","searchDatabaseTagsDesc",g);d.target.remove();searchAlbumgrid(document.getElementById("searchDatabaseStr").value)}},!1);domCache.searchstr.addEventListener("keyup",function(d){if("Escape"===d.key)this.blur();else if("Enter"===d.key&&settings.featAdvsearch)if(""!==this.value){d=document.getElementById("searchMatch");
var g=document.createElement("button");g.classList.add("btn","btn-light","mr-2");g.setAttribute("data-filter",encodeURI(app.current.filter+" "+d.options[d.selectedIndex].value+" '"+this.value+"'"));g.innerHTML=e(app.current.filter)+" "+e(d.options[d.selectedIndex].value)+" '"+e(this.value)+'\'<span class="ml-2 badge badge-secondary">&times;</span>';this.value="";domCache.searchCrumb.appendChild(g)}else doSearch(this.value);else doSearch(this.value)},!1);domCache.searchCrumb.addEventListener("click",
function(d){d.preventDefault();d.stopPropagation();if("SPAN"===d.target.nodeName)d.target.parentNode.remove(),doSearch("");else if("BUTTON"===d.target.nodeName){var g=decodeURI(d.target.getAttribute("data-filter"));domCache.searchstr.value=g.substring(g.indexOf("'")+1,g.length-1);var k=g.substring(0,g.indexOf(" "));selectTag("searchtags","searchtagsdesc",k);g=g.substring(g.indexOf(" ")+1);g=g.substring(0,g.indexOf(" "));document.getElementById("searchMatch").value=g;d.target.remove();doSearch(domCache.searchstr.value)}},
!1);document.getElementById("searchMatch").addEventListener("change",function(){doSearch(domCache.searchstr.value)},!1);document.getElementById("SearchList").getElementsByTagName("tr")[0].addEventListener("click",function(d){if(settings.featAdvsearch&&"TH"===d.target.nodeName&&""!==d.target.innerHTML){var g=d.target.getAttribute("data-col");if("Duration"!==g){var k=app.current.sort,l=!0;if(k===g||k==="-"+g)0===k.indexOf("-")?(l=!0,g=k.substring(1)):l=!1;!1===l?(k="-"+g,l=!0):(l=!1,k=g);for(var m=
document.getElementById("SearchList").getElementsByClassName("sort-dir"),n=0;n<m.length;n++)m[n].remove();app.current.sort=k;d.target.innerHTML=t(g)+'<span class="sort-dir material-icons pull-right">'+(!0===l?"arrow_drop_up":"arrow_drop_down")+"</span>";appGoto(app.current.app,app.current.tab,app.current.view,app.current.page,app.current.filter,app.current.sort,"-",app.current.search)}}},!1);document.getElementById("inputScriptArgument").addEventListener("keyup",function(d){"Enter"===d.key&&(d.preventDefault(),
d.stopPropagation(),addScriptArgument())},!1);document.getElementById("selectScriptArguments").addEventListener("click",function(d){"OPTION"===d.target.nodeName&&removeScriptArgument(d)},!1);document.getElementsByTagName("body")[0].addEventListener("click",function(){hideMenu()},!1);dragAndDropTable("QueueCurrentList");dragAndDropTable("BrowsePlaylistsDetailList");dragAndDropTableHeader("QueueCurrent");dragAndDropTableHeader("QueueLastPlayed");dragAndDropTableHeader("QueueJukebox");dragAndDropTableHeader("Search");
dragAndDropTableHeader("BrowseFilesystem");dragAndDropTableHeader("BrowsePlaylistsDetail");dragAndDropTableHeader("BrowseDatabaseDetail");window.addEventListener("focus",function(){sendAPI("MPD_API_PLAYER_STATE",{},parseState)},!1);document.addEventListener("keydown",function(d){},!1);a=document.getElementsByTagName("table");for(b=0;b<a.length;b++)a[b].setAttribute("tabindex",0),a[b].addEventListener("keydown",function(d){navigateTable(this,d.key)},!1);var h="";Object.keys(themes).forEach(function(d){h+=
'<option value="'+e(d)+'">'+t(themes[d])+"</option>"});document.getElementById("selectTheme").innerHTML=h;window.addEventListener("beforeunload",function(){webSocketClose()});document.getElementById("alertLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(d){d.stopPropagation();d.preventDefault();clickCheckLocalPlayerState(d)},!1);document.getElementById("errorLocalPlayback").getElementsByTagName("a")[0].addEventListener("click",function(d){d.stopPropagation();d.preventDefault();
clickCheckLocalPlayerState(d)},!1);document.getElementById("localPlayer").addEventListener("click",function(d){d.stopPropagation()});document.getElementById("localPlayer").addEventListener("canplay",function(){logDebug("localPlayer event: canplay");document.getElementById("alertLocalPlayback").classList.add("hide");document.getElementById("errorLocalPlayback").classList.add("hide")});document.getElementById("localPlayer").addEventListener("error",function(){logError("localPlayer event: error");document.getElementById("errorLocalPlayback").classList.remove("hide")});
checkInit()}window.onerror=function(a,b,c){logError("JavaScript error: "+a+" ("+b+": "+c+")");4<=settings.loglevel&&(!0===appInited?showNotification(t("JavaScript error"),a+" ("+b+": "+c+")","","danger"):showAppInitAlert(t("JavaScript error")+": "+a+" ("+b+": "+c+")"));return!0};window.onresize=function(){"Playback"===app.current.app&&calcBoxHeight()};appInitStart();
...
function setGridPlayback(){var a=["col-md-6","d-none","d-md-block"];if("Playback"===app.current.app){document.getElementById("row1").classList.add("row");document.getElementById("col1").classList.add("col-md-6");var b;(b=document.getElementById("col2").classList).add.apply(b,$jscomp.arrayFromIterable(a));document.getElementById("cardQueueMini").classList.remove("hide");document.getElementById("cardBrowse").classList.remove("hide");document.getElementById("cardBrowseDatabase").classList.remove("hide");
document.getElementById("searchDatabaseTagsDesc").classList.add("hide")}else{document.getElementById("row1").classList.remove("row");document.getElementById("col1").classList.remove("col-md-6");var c;(c=document.getElementById("col2").classList).remove.apply(c,$jscomp.arrayFromIterable(a));document.getElementById("cardQueueMini").classList.add("hide");document.getElementById("searchDatabaseTagsDesc").classList.remove("hide");document.getElementById("viewListDatabaseBox").style.height="";document.getElementById("viewDetailDatabaseBox").style.height=
""}}function setAppState(a,b,c,f,h){var d="Browse"===app.current.app?"Playback":"Browse";a={page:a,filter:b,sort:c,tag:f,search:h};a.scrollPos=app.apps[d].tabs.Database.views[app.current.view].scrollPos;app.apps[d].tabs.Database.active=app.current.view;app.apps[d].tabs.Database.views[app.current.view]=a}
function getQueueMini(a){var b=["Pos","Title","Artist","Album","Duration"];if(void 0!==a&&-1!==a)sendAPI("MPD_API_QUEUE_MINI",{pos:a,cols:b},parseQueueMini);else if(void 0===a){a=document.getElementById("QueueMiniList");b=a.getElementsByTagName("tbody")[0];for(var c=b.rows.length-1;0<=c;c--)b.deleteRow(c);a.classList.add("opacity05")}sendAPI("MPD_API_QUEUE_LIST",{offset:0,cols:[]},parseQueueList)}
function parseQueueMini(a){var b=["Pos","Title","Artist","Album","Duration"],c=a.result.returnedEntities,f=document.getElementById("QueueMiniList"),h=0;f.setAttribute("data-version",a.result.queueVersion);var d=f.getElementsByTagName("tbody")[0];f=d.getElementsByTagName("tr");for(var g=0;g<c;g++){a.result.data[g].Duration=beautifySongDuration(a.result.data[g].Duration);a.result.data[g].Pos++;var k=document.createElement("tr");k.setAttribute("data-trackid",a.result.data[g].id);k.setAttribute("id",
"queueMiniTrackId"+a.result.data[g].id);k.setAttribute("data-songpos",a.result.data[g].Pos);k.setAttribute("data-duration",a.result.data[g].Duration);k.setAttribute("data-uri",a.result.data[g].uri);k.setAttribute("tabindex",0);for(var l="",m=0;m<b.length;m++)l+='<td data-col="'+b[m]+'">'+e(a.result.data[g][b[m]])+"</td>";k.innerHTML=l;g<f.length?h=!0===replaceTblRow(f[g],k)?g:h:d.append(k)}for(a=f.length-1;a>=c;a--)f[a].remove();document.getElementById("QueueMiniList").classList.remove("opacity05")}
...
function swapView(){var a=document.getElementById("BrowseDatabaseCards").parentNode,b=document.getElementById("BrowseDatabaseAlbumList").parentNode;a.classList.contains("hide")?(b.classList.add("hide"),a.classList.remove("hide")):(a.classList.add("hide"),b.classList.remove("hide"))}
function setDatabaseList(){"Album"===app.current.tag?document.getElementById("btnSwapView").removeAttribute("disabled"):(document.getElementById("btnSwapView").setAttribute("disabled","disabled"),document.getElementById("BrowseDatabaseCards").parentNode.classList.contains("hide")&&swapView())}
function parseDatabaseAlbum(a){for(var b=a.result.returnedEntities,c=document.getElementById("BrowseDatabaseAlbumList").getElementsByTagName("tbody")[0],f=c.getElementsByTagName("tr"),h=0,d=0;d<b;d++){var g=document.createElement("tr"),k="";g.setAttribute("data-type","dir");var l=dirname(a.result.data[d].FirstSongUri);g.setAttribute("data-uri",encodeURI(l));g.setAttribute("tabindex",0);g.setAttribute("data-album",a.result.data[d].Album);g.setAttribute("data-albumartist",a.result.data[d].AlbumArtist);
g.setAttribute("data-name",a.result.data[d].Album);k+='<td data-col="Type"><span class="material-icons">album</span></td>';k+='<td data-col="Album">'+e(a.result.data[d].Album)+"</td>";k+='<td data-col="AlbumArtist">'+e(a.result.data[d].AlbumArtist)+"</td>";k+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a></td>";g.innerHTML=k;d<f.length?h=!0===replaceTblRow(f[d],g)?d:h:c.append(g)}for(a=f.length-1;a>=b;a--)f[a].remove();0===b&&(c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="3">'+
t("Empty list")+"</td></tr>");document.getElementById("BrowseDatabaseAlbumList").classList.remove("opacity05")}
function calcBoxHeight(){if("Playback"===app.current.app){var a=document.getElementById("cardPlayback").offsetHeight,b=document.getElementById("cardQueueMini").offsetHeight,c=document.getElementById("BrowseDatabaseButtons").offsetHeight,f=document.getElementById("searchDatabaseCrumb").offsetHeight,h=document.getElementById("PlaybackButtonsTop").offsetHeight;document.getElementById("viewListDatabaseBox").style.height=a+b-c-f-17+"px";document.getElementById("viewDetailDatabaseBox").style.height=a+b-
h-1+"px"}}
function parsePlaylists(a){"All"===app.current.view?(document.getElementById("BrowsePlaylistsAllList").classList.remove("hide"),document.getElementById("BrowsePlaylistsDetailList").classList.add("hide"),document.getElementById("btnBrowsePlaylistsAll").parentNode.classList.add("hide"),document.getElementById("playlistContentBtns").classList.add("hide"),document.getElementById("smartPlaylistContentBtns").classList.add("hide"),document.getElementById("btnAddSmartpls").parentNode.classList.remove("hide"),document.getElementById("BrowseNavPlaylists").parentNode.classList.remove("hide")):
...
function hideMenu(){var a=document.querySelector("[data-popover]");a&&((new BSN.Popover(a,{})).hide(),a.removeAttribute("data-popover"),a.parentNode.parentNode.classList.contains("selected")?focusTable(void 0,a.parentNode.parentNode.parentNode.parentNode):("Browse"!==app.current.app&&"Playback"!==app.current.app||"Database"!==app.current.tab)&&"Home"===app.current.app&&focusTable(void 0,a.parentNode.parentNode.parentNode.parentNode))}
function showMenu(a,b){b.preventDefault();b.stopPropagation();hideMenu();"TH"===a.parentNode.nodeName?showMenuTh(a):showMenuTd(a)}
function showMenuTh(a){var b=("Playback"===app.current.app?"Browse":app.current.app)+(void 0!==app.current.tab?app.current.tab:"")+(void 0!==app.current.view?app.current.view:"");var c='<form class="p-2" id="colChecklist'+b+'">'+setColsChecklist(b);c+='<button class="btn btn-success btn-block btn-sm mt-2">'+t("Apply")+"</button>";new BSN.Popover(a,{trigger:"click",delay:0,dismissible:!0,template:'<div class="popover" role="tooltip"><div class="arrow"></div><div class="popover-content" id="'+b+'ColsDropdown">'+
(c+"</form></div></div>"),content:" "});c=a.Popover;null===a.getAttribute("data-init")&&(a.setAttribute("data-init","true"),a.addEventListener("shown.bs.popover",function(f){f.target.setAttribute("data-popover","true");document.getElementById("colChecklist"+b).addEventListener("click",function(h){"BUTTON"===h.target.nodeName&&h.target.classList.contains("material-icons")?(toggleBtnChk(h.target),h.preventDefault(),h.stopPropagation()):"BUTTON"===h.target.nodeName&&(h.preventDefault(),saveCols(b))},
!1)},!1));c.show()}
...
var d="";if("Browse"===app.current.app&&"Filesystem"===app.current.tab||"Search"===app.current.app||("Browse"===app.current.app||"Playback"===app.current.app)&&"Database"===app.current.tab&&"Detail"===app.current.view){if(d+=addMenuItem({cmd:"appendPlayQueue",options:[b,c,f]},t("Play"))+addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Add to queue"))+("song"===b?addMenuItem({cmd:"appendAfterQueue",options:[b,c,h,f]},t("Add after current playing song")):"")+addMenuItem({cmd:"replaceQueue",options:[b,
c,f]},t("Replace queue"))+("plist"!==b&&"smartpls"!==b&&!0===settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+("song"===b?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"")+("plist"===b||"smartpls"===b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):"")+("plist"!==b&&"smartpls"!==b||!0!==settings.featHome?"":addMenuItem({cmd:"addPlistToHome",options:[c,f]},t("Add to homescreen"))),"Filesystem"===app.current.tab&&
(d+=("dir"===b&&settings.featBookmarks?addMenuItem({cmd:"showBookmarkSave",options:[-1,f,c,b]},t("Add bookmark")):"")+("dir"===b?addMenuItem({cmd:"updateDB",options:[dirname(c),!0]},t("Update directory")):"")+("dir"===b?addMenuItem({cmd:"rescanDB",options:[dirname(c),!0]},t("Rescan directory")):"")),"Search"===app.current.app){h=a.parentNode.parentNode.getAttribute("data-album");var g=a.parentNode.parentNode.getAttribute("data-albumartist");null!==h&&null!==g&&"undefined"!==h&&"undefined"!==g?(h=
decodeURI(h),g=decodeURI(g),d+='<div class="dropdown-divider"></div><a class="dropdown-item" id="advancedMenuLink" data-toggle="collapse" href="#advancedMenu"><span class="material-icons material-icons-left">keyboard_arrow_right</span>Album actions</a><div class="collapse" id="advancedMenu">'+addMenuItem({cmd:"_addAlbum",options:["appendPlayQueue",g,h]},t("Play"))+addMenuItem({cmd:"_addAlbum",options:["appendQueue",g,h]},t("Add to queue"))+addMenuItem({cmd:"_addAlbum",options:["replaceQueue",g,h]},
t("Replace queue"))+(!0===settings.featPlaylists?addMenuItem({cmd:"_addAlbum",options:["addPlaylist",g,h]},t("Add to playlist")):"")+"</div>"):(c=dirname(c),d+='<div class="dropdown-divider"></div><a class="dropdown-item" id="advancedMenuLink" data-toggle="collapse" href="#advancedMenu"><span class="material-icons material-icons-left">keyboard_arrow_right</span>Folder actions</a><div class="collapse" id="advancedMenu">'+addMenuItem({cmd:"appendPlayQueue",options:[b,c,f]},t("Play"))+addMenuItem({cmd:"appendQueue",
options:[b,c,f]},t("Add to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+(!0===settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+"</div>")}}else if(("Browse"===app.current.app||"Playback"===app.current.app)&&"Database"===app.current.tab&&"List"===app.current.view)b="A"===a.nodeName?decodeURI(a.parentNode.parentNode.getAttribute("data-albumartist")):decodeURI(a.parentNode.getAttribute("data-albumartist")),f="A"===a.nodeName?
decodeURI(a.parentNode.parentNode.getAttribute("data-album")):decodeURI(a.parentNode.getAttribute("data-album")),d+=addMenuItem({cmd:"_addAlbum",options:["appendPlayQueue",b,f]},t("Play"))+addMenuItem({cmd:"_addAlbum",options:["appendQueue",b,f]},t("Add to queue"))+addMenuItem({cmd:"_addAlbum",options:["replaceQueue",b,f]},t("Replace queue"))+(!0===settings.featPlaylists?addMenuItem({cmd:"_addAlbum",options:["addPlaylist",b,f]},t("Add to playlist")):"");else if("Browse"===app.current.app&&"Playlists"===
app.current.tab&&"All"===app.current.view)d+=addMenuItem({cmd:"appendPlayQueue",options:[b,c,f]},t("Play"))+addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Add to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+(!0===settings.smartpls&&"smartpls"===b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):addMenuItem({cmd:"playlistDetails",options:[c]},t("Edit playlist")))+(!0===settings.smartpls&&"smartpls"===b?addMenuItem({cmd:"showSmartPlaylist",options:[c]},
t("Edit smart playlist")):"")+(!0===settings.smartpls&&"smartpls"===b?addMenuItem({cmd:"updateSmartPlaylist",options:[c]},t("Update smart playlist")):"")+addMenuItem({cmd:"showRenamePlaylist",options:[c]},t("Rename playlist"))+addMenuItem({cmd:"showDelPlaylist",options:[c]},t("Delete playlist"))+(!0===settings.featHome?addMenuItem({cmd:"addPlistToHome",options:[c,f]},t("Add to homescreen")):"");else if("Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view)h=document.getElementById("BrowsePlaylistsDetailList"),
d+=addMenuItem({cmd:"appendPlayQueue",options:[b,c,f]},t("Play"))+addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Add to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+("false"===h.getAttribute("data-ro")?addMenuItem({cmd:"removeFromPlaylist",options:[h.getAttribute("data-uri"),a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove")):"")+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+(-1===c.indexOf("http")?
addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"Current"===app.current.tab)d+=addMenuItem({cmd:"delQueueSong",options:["single",a.parentNode.parentNode.getAttribute("data-trackid")]},t("Remove"))+addMenuItem({cmd:"delQueueSong",options:["range",0,a.parentNode.parentNode.getAttribute("data-songpos")]},t("Remove all upwards"))+addMenuItem({cmd:"delQueueSong",options:["range",parseInt(a.parentNode.parentNode.getAttribute("data-songpos"))-1,-1]},
t("Remove all downwards"))+(settings.featPlaylists?addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"LastPlayed"===app.current.tab)d+=addMenuItem({cmd:"appendPlayQueue",options:[b,c,f]},t("Play"))+addMenuItem({cmd:"appendQueue",options:[b,c,f]},t("Add to queue"))+addMenuItem({cmd:"replaceQueue",options:[b,c,f]},t("Replace queue"))+(settings.featPlaylists?
addMenuItem({cmd:"showAddToPlaylist",options:[c,""]},t("Add to playlist")):"")+(-1===c.indexOf("http")?addMenuItem({cmd:"songDetails",options:[c]},t("Song details")):"");else if("Queue"===app.current.app&&"Jukebox"===app.current.tab)d+=addMenuItem({cmd:"songDetails",options:[c]},t("Song details"))+addMenuItem({cmd:"delQueueJukeboxSong",options:[a.parentNode.parentNode.getAttribute("data-pos")]},t("Remove"));else if("Home"===app.current.app){h=parseInt(a.parentNode.getAttribute("data-pos"));g=JSON.parse(a.parentNode.getAttribute("data-href"));
var k="";"replaceQueue"===g.cmd&&"plist"===g.options[0]?(b="plist",c=g.options[1],k=t("Add and play playlist"),f=t("Playlist")):"appGoto"===g.cmd?(b="view",k=t("Goto view"),f=t("View")):"execScriptFromOptions"===g.cmd&&(b="script",k=t("Execute script"),f=t("Script"));d+='<h6 class="dropdown-header">'+f+"</h6>"+addMenuItem({cmd:"executeHomeIcon",options:[h]},k)+("plist"===b?addMenuItem({cmd:"playlistDetails",options:[c]},t("View playlist")):"")+addMenuItem({cmd:"editHomeIcon",options:[h]},t("Edit home icon"))+
addMenuItem({cmd:"duplicateHomeIcon",options:[h]},t("Duplicate home icon"))+addMenuItem({cmd:"deleteHomeIcon",options:[h]},t("Delete home icon"))}new BSN.Popover(a,{trigger:"click",delay:0,dismissible:!0,template:'<div class="popover" role="tooltip"><div class="arrow"></div><div class="popover-content">'+d+"</div></div>",content:" "});d=a.Popover;null===a.getAttribute("data-init")&&(a.setAttribute("data-init","true"),a.addEventListener("shown.bs.popover",function(l){l.target.setAttribute("data-popover",
"true");document.getElementsByClassName("popover-content")[0].addEventListener("click",function(n){n.preventDefault();n.stopPropagation();"A"===n.target.nodeName&&(n=n.target.getAttribute("data-href"))&&(n=JSON.parse(b64DecodeUnicode(n)),parseCmd(l,n),hideMenu())},!1);document.getElementsByClassName("popover-content")[0].addEventListener("keydown",function(n){n.preventDefault();n.stopPropagation();if("ArrowDown"===n.key||"ArrowUp"===n.key){var p=this.getElementsByTagName("a");p=Array.prototype.slice.call(p);
var q=p.indexOf(document.activeElement);do if(q="ArrowUp"===n.key?1<q?q-1:0:"ArrowDown"===n.key?q<p.length-1?q+1:q:q,0===q||q===p.length-1)break;while(!p[q].offsetHeight);p[q]&&p[q].focus()}else"Enter"===n.key?n.target.click():"Escape"===n.key&&hideMenu()},!1);var m=document.getElementById("advancedMenuLink");m&&(m.addEventListener("click",function(){var n=this.getElementsByTagName("span")[0];n.innerText="keyboard_arrow_right"===n.innerText?"keyboard_arrow_down":"keyboard_arrow_right"},!1),new BSN.Collapse(m));
document.getElementsByClassName("popover-content")[0].firstChild.focus()},!1));d.show()}
...
k.setAttribute("tabindex",0);var l="";l+='<td data-col="Img"><img class="covergrid-header mr-0"/></td>';for(var m=0;m<settings.colsQueueCurrent.length;m++)l+='<td data-col="'+settings.colsQueueCurrent[m]+'">'+e(a.result.data[g][settings.colsQueueCurrent[m]])+"</td>";l+='<td data-col="Action"><a href="#" class="material-icons color-darkgrey">'+ligatureMore+"</a></td>";k.innerHTML=l;g<d.length?h=!0===replaceTblRow(d[g],k)?g:h:c.append(k);"IntersectionObserver"in window?(new IntersectionObserver(setRowImage,
{root:null,rootMargin:"0px"})).observe(k):k.firstChild.firstChild.src=subdir+"/albumart/"+a.result.data[g].uri}for(g=d.length-1;g>=b;g--)d[g].remove();d=settings.colsQueueCurrent.length;d--;"MPD_API_QUEUE_SEARCH"===a.result.method&&0===b?c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+d+'">'+t("No results, please refine your search")+"</td></tr>":"MPD_API_QUEUE_ADD_TRACK"===a.result.method&&0===b&&(c.innerHTML='<tr><td><span class="material-icons">error_outline</span></td><td colspan="'+
d+'">'+t("Empty queue")+"</td></tr>");!0===f&&focusTable(h);setPagination(a.result.totalEntities,a.result.returnedEntities);document.getElementById("QueueCurrentList").classList.remove("opacity05");scrollToPosY(app.current.scrollPos)}}function setRowImage(a,b){a.forEach(function(c){if(0<c.intersectionRatio){b.unobserve(c.target);var f=c.target.getAttribute("data-uri");c.target.firstChild.firstChild.src=subdir+"/albumart/"+f}})}
...
function appendPlayQueue(a,b,c){switch(a){case "song":sendAPI("MPD_API_QUEUE_ADD_TRACK_PLAY",{uri:b});showNotification(t("%{name} added to queue",{name:c}),"","","success");break;case "dir":sendAPI("MPD_API_QUEUE_ADD_DIR_PLAY",{uri:b});showNotification(t("%{name} added to queue",{name:c}),"","","success");break;case "plist":case "smartpls":sendAPI("MPD_API_QUEUE_ADD_PLAYLIST_PLAY",{plist:b}),showNotification(t("%{name} added to queue",{name:c}),"","","success")}}
...
function parseScriptList(a){var b=document.createElement("optgroup");b.setAttribute("data-value","script");b.setAttribute("label",t("Script"));var c="",f="",h=a.result.data.length;if(0<h){a.result.data.sort(function(l,m){return l.metadata.order-m.metadata.order});for(var d=0,g=0;g<h;g++){var k="";if(0<a.result.data[g].metadata.arguments.length){for(k=0;k<a.result.data[g].metadata.arguments.length;k++)a.result.data[g].metadata.arguments[k]=e(a.result.data[g].metadata.arguments[k]);k='"'+a.result.data[g].metadata.arguments.join('","')+
'"'}0<a.result.data[g].metadata.order&&(0===d&&(c=4<h?"":'<div class="dropdown-divider"></div>'),d++,c+='<a class="dropdown-item text-light alwaysEnabled" href="#" data-href=\'{"script": "'+e(a.result.data[g].name)+'", "arguments": ['+k+"]}'>"+e(a.result.data[g].name)+"</a>");f+='<tr data-script="'+encodeURI(a.result.data[g].name)+'"><td>'+e(a.result.data[g].name)+'</td><td data-col="Action">'+(!0===settings.featScripteditor?'<a href="#" title="'+t("Delete")+'" data-action="delete" class="material-icons color-darkgrey">delete</a>':
"")+'<a href="#" title="'+t("Execute")+'" data-action="execute" class="material-icons color-darkgrey"  data-href=\'{"script": "'+e(a.result.data[g].name)+'", "arguments": ['+k+']}\'>play_arrow</a><a href="#" title="'+t("Add to homescreen")+'" data-action="add2home" class="material-icons color-darkgrey"  data-href=\'{"script": "'+e(a.result.data[g].name)+'", "arguments": ['+k+"]}'>add_to_home_screen</a></td></tr>";b.innerHTML+='<option data-arguments=\'{"arguments":['+k+"]}' value=\""+e(a.result.data[g].name)+
'">'+e(a.result.data[g].name)+"</option>"}document.getElementById("listScriptsList").innerHTML=f}else document.getElementById("listScriptsList").innerHTML='<tr class="not-clickable"><td><span class="material-icons">error_outline</span></td><td colspan="2">'+t("Empty list")+"</td></tr>";document.getElementById("scripts").innerHTML=c;4<h?(document.getElementById("navScripting").classList.remove("hide"),document.getElementById("scripts").classList.add("collapse","menu-indent")):(document.getElementById("navScripting").classList.add("hide"),
document.getElementById("scripts").classList.remove("collapse","menu-indent"));document.getElementById("selectTriggerScript").innerHTML=b.innerHTML;(a=document.getElementById("selectTimerAction").querySelector('optgroup[data-value="script"]'))?a.replaceWith(b):document.getElementById("selectTimerAction").appendChild(b)}function execScriptFromOptions(a,b){b=void 0!==b&&""!==b?b.split(","):[];execScript(JSON.stringify({script:a,arguments:b}))}
...
function doSearch(a){if(settings.featAdvsearch){for(var b="(",c=domCache.searchCrumb.children,f=0;f<c.length;f++)b+="("+decodeURI(c[f].getAttribute("data-filter"))+")",""!==a&&(b+=" AND ");""!==a?(c=document.getElementById("searchMatch"),b+="("+app.current.filter+" "+c.options[c.selectedIndex].value+" '"+a+"'))"):b+=")";2>=b.length&&(b="");appGoto("Search",void 0,void 0,"0",app.current.filter,app.current.sort,"-",b)}else appGoto("Search",void 0,void 0,"0",app.current.filter,app.current.sort,"-",a)}
...
function addAllFromSearchPlist(a,b,c,f){f=void 0===f?!1:f;null===b&&(b=app.current.search);settings.featAdvsearch?sendAPI("MPD_API_DATABASE_SEARCH_ADV",{plist:a,sort:"",sortdesc:!1,expression:b,offset:0,cols:settings.colsSearch,replace:c,play:f}):sendAPI("MPD_API_DATABASE_SEARCH",{plist:a,filter:app.current.filter,searchstr:b,offset:0,cols:settings.colsSearch,replace:c,play:f})}
...
function getMpdSettings(a){if(""!==a&&a.result)settingsNew=a.result,document.getElementById("splashScreenAlert").innerText=t("Fetch Streamer settings"),sendAPI("MPD_API_SETTINGS_GET",{},joinSettings,!0);else return settingsParsed="error",!1===appInited&&showAppInitAlert(""===a?t("Can not parse settings"):t(a.error.message)),!1}
...
settings.smartplsSort;domCache.volumeBar.setAttribute("min",settings.volumeMin);domCache.volumeBar.setAttribute("max",settings.volumeMax);!0===settings.featLocalplayer&&setLocalPlayerUrl();"auto"===settings.musicDirectory?(document.getElementById("selectMusicDirectory").value=settings.musicDirectory,document.getElementById("inputMusicDirectory").value=void 0!==settings.musicDirectoryValue?settings.musicDirectoryValue:"",document.getElementById("inputMusicDirectory").setAttribute("readonly","readonly")):
...
"Current"===app.current.tab?getQueue():"Queue"===app.current.app&&"LastPlayed"===app.current.tab?appRoute():"Queue"===app.current.app&&"Jukebox"===app.current.tab?appRoute():"Search"===app.current.app?appRoute():"Browse"===app.current.app&&"Filesystem"===app.current.tab?appRoute():"Browse"===app.current.app&&"Playlists"===app.current.tab&&"Detail"===app.current.view?appRoute():"Browse"!==app.current.app&&"Playback"!==app.current.app||"Database"!==app.current.tab||""===app.current.search||(calcBoxHeight(),
appRoute());i18nHtml(document.getElementsByTagName("body")[0]);checkConsume();!0===settings.mediaSession&&"mediaSession"in navigator?(navigator.mediaSession.setActionHandler("play",clickPlay),navigator.mediaSession.setActionHandler("pause",clickPlay),navigator.mediaSession.setActionHandler("stop",clickStop),navigator.mediaSession.setActionHandler("seekbackward",seekRelativeBackward),navigator.mediaSession.setActionHandler("seekforward",seekRelativeForward),navigator.mediaSession.setActionHandler("previoustrack",
clickPrev),navigator.mediaSession.setActionHandler("nexttrack",clickNext),navigator.mediaSession.setPositionState||logDebug("mediaSession.setPositionState not supported by browser")):logDebug("mediaSession not supported by browser");parseIdeonSettings();settingsParsed="true"}
...
settings.colsPlayback[b]&&null!==lastSongObj[tagAlbumArtist]&&(a+='data-albumartist="'+encodeURI(lastSongObj[tagAlbumArtist])+'"'),a+="><small>"+t(settings.colsPlayback[b])+" &bull; </small><span",settings.browsetags.includes(settings.colsPlayback[b])&&(a+=' class="clickable"'),a+=">",a="Duration"===settings.colsPlayback[b]?a+(lastSongObj[settings.colsPlayback[b]]?beautifySongDuration(lastSongObj[settings.colsPlayback[b]]):""):"LastModified"===settings.colsPlayback[b]?a+(lastSongObj[settings.colsPlayback[b]]?
localeDate(lastSongObj[settings.colsPlayback[b]]):""):"Fileformat"===settings.colsPlayback[b]?a+(lastState?fileformat(lastState.audioFormat):""):a+(lastSongObj[settings.colsPlayback[b]]?e(lastSongObj[settings.colsPlayback[b]]):""),a+="</span></div>";document.getElementById("cardPlaybackTags").innerHTML=a;(a=document.getElementById("currentLyrics"))&&lastSongObj.uri&&(b=a.getElementsByTagName("small")[0],b.classList.add("clickable"),b.addEventListener("click",function(k){k.target.parentNode.children[1].classList.toggle("expanded")},
...
initTagMultiSelect("inputSearchTags","listSearchTags",settings.tags,settings.searchtags);initTagMultiSelect("inputBrowseTags","listBrowseTags",settings.tags,settings.browsetags);initTagMultiSelect("inputGeneratePlsTags","listGeneratePlsTags",settings.browsetags,settings.generatePlsTags)}function resetSettings(){resetFlag=!0;modalSettings.hide()}
...
validateInt(f)||(b=!1);f=document.getElementById("inputCoverimageSize");validateInt(f)||(b=!1);f=document.getElementById("inputCoverimageName");validateFilenameList(f)||(b=!1);f=document.getElementById("inputBookletName");validateFilename(f)||(b=!1);f=document.getElementById("inputMaxElementsPerPage");validateInt(f)||(b=!1);!0===isMobile&&(d=document.getElementById("inputScaleRatio"),validateFloat(d)?(scale=parseFloat(d.value),setViewport(!0)):b=!1);200<parseInt(f.value)&&(b=!1);f=document.getElementById("inputLastPlayedCount");
validateInt(f)||(b=!1);document.getElementById("btnLoveEnable").classList.contains("active")&&(f=document.getElementById("inputLoveChannel"),d=document.getElementById("inputLoveMessage"),validateNotBlank(f)&&validateNotBlank(d)||(b=!1));!0===settings.featMixramp&&(f=document.getElementById("inputMixrampdb"),f.getAttribute("disabled")||validateFloat(f)||(b=!1),f=document.getElementById("inputMixrampdelay"),f.getAttribute("disabled")||("nan"===f.value&&(f.value="-1"),validateFloat(f)||(b=!1)));f=document.getElementById("inputSmartplsInterval");
validateInt(f)||(b=!1);f=3600*document.getElementById("inputSmartplsInterval").value;d=getBtnGroupValue("btnSingleGroup");var g=getBtnGroupValue("btnJukeboxModeGroup"),k=getBtnGroupValue("btnReplaygainGroup"),l=document.getElementById("selectJukeboxUniqueTag");l=l.options[l.selectedIndex].value;var m=document.getElementById("selectJukeboxPlaylist");m=m.options[m.selectedIndex].value;"2"===g&&(l="Album");"1"===g&&!1===settings.featSearchwindow&&"Database"===m&&(b=!1,document.getElementById("warnJukeboxPlaylist").classList.remove("hide"));
if(!0===b){b=document.getElementById("selectLocale");var n=document.getElementById("selectTheme");sendAPI("MYMPD_API_SETTINGS_SET",{consume:document.getElementById("btnConsume").classList.contains("active")?1:0,random:document.getElementById("btnRandom").classList.contains("active")?1:0,single:parseInt(d),repeat:document.getElementById("btnRepeat").classList.contains("active")?1:0,replaygain:k,crossfade:document.getElementById("inputCrossfade").value,mixrampdb:!0===settings.featMixramp?document.getElementById("inputMixrampdb").value:
...
bookmarks:document.getElementById("btnBookmarks").classList.contains("active")?!0:!1,maxElementsPerPage:document.getElementById("inputMaxElementsPerPage").value,stickers:document.getElementById("btnStickers").classList.contains("active")?!0:!1,lastPlayedCount:document.getElementById("inputLastPlayedCount").value,smartpls:document.getElementById("btnSmartpls").classList.contains("active")?!0:!1,smartplsPrefix:document.getElementById("inputSmartplsPrefix").value,smartplsInterval:f,smartplsSort:document.getElementById("selectSmartplsSort").value,
...
[],f=0;f<settings[a].length;f++)b.includes(settings[a][f])&&c.push(settings[a][f]);settings[a]=c;logDebug("Columns for "+a+": "+c)}
...
if(lastState&&lastState.currentSongId!==a){if(c=document.getElementById("queueTrackId"+lastState.currentSongId)){if(f=c.querySelector("[data-col=Duration]"))f.innerText=c.getAttribute("data-duration");if(f=c.querySelector("[data-col=Pos]"))f.classList.remove("material-icons"),f.innerText=c.getAttribute("data-songpos");c.classList.remove("font-weight-bold")}if(c=document.getElementById("queueMiniTrackId"+lastState.currentSongId)){if(f=c.querySelector("[data-col=Duration]"))f.innerText=c.getAttribute("data-duration");
if(f=c.querySelector("[data-col=Pos]"))f.classList.remove("material-icons"),f.innerText=c.getAttribute("data-songpos");c.classList.remove("font-weight-bold")}}if(c=document.getElementById("queueTrackId"+a)){if(f=c.querySelector("[data-col=Duration]"))f.innerHTML=b;(f=c.querySelector("[data-col=Pos]"))&&!f.classList.contains("material-icons")&&(f.classList.add("material-icons"),f.innerText="play_arrow");c.classList.add("font-weight-bold")}if(c=document.getElementById("queueMiniTrackId"+a)){if(a=c.querySelector("[data-col=Duration]"))a.innerHTML=
b;(a=c.querySelector("[data-col=Pos]"))&&!a.classList.contains("material-icons")&&(a.classList.add("material-icons"),a.innerText="play_arrow");c.classList.add("font-weight-bold")}progressTimer&&clearTimeout(progressTimer);"play"===playstate&&(progressTimer=setTimeout(function(){currentSong.elapsedTime++;requestAnimationFrame(function(){setCounter(currentSong.currentSongId,currentSong.totalTime,currentSong.elapsedTime)})},1E3))}
function parseState(a){if(JSON.stringify(a.result)===JSON.stringify(lastState))toggleUI();else{parseUpdateQueue(a);parseVolume(a);setCounter(a.result.currentSongId,a.result.totalTime,a.result.elapsedTime);lastState&&lastState.currentSongId===a.result.currentSongId&&lastState.queueVersion===a.result.queueVersion||sendAPI("MPD_API_PLAYER_CURRENT_SONG",{},songChange);if("-1"===a.result.songPos){domCache.currentTitle.innerText="Not playing";document.title="Ideon";domCache.footerTitle.innerText="";domCache.footerTitle.removeAttribute("title");
domCache.footerTitle.classList.remove("clickable");domCache.footerCover.classList.remove("clickable");clearCurrentCover();!0===settings.bgCover&&clearBackgroundImage();for(var b=document.getElementById("cardPlaybackTags").getElementsByTagName("span"),c=0;c<b.length;c++)b[c].innerText=""}else if(b=document.getElementById("currentFileformat"))b.getElementsByTagName("span")[0].innerText=fileformat(a.result.audioFormat);lastState=a.result;!1!==settings.mpdConnected&&!1!==uiEnabled||getSettings(!0)}}
...
function songChange(a){"Playback"===app.current.app&&getQueueMini(a.result.pos);var b=a.result.Title+":"+a.result.Artist+":"+a.result.Album+":"+a.result.uri+":"+a.result.currentSongId;if(lastSong!==b){var c="",f="",h="";mediaSessionSetMetadata(a.result.Title,a.result.Artist,a.result.Album,a.result.uri);setCurrentCover(a.result.uri);!0===settings.bgCover&&!0===settings.featCoverimage&&setBackgroundImage(a.result.uri);domCache.footerArtist.classList.remove("clickable");domCache.footerAlbum.classList.remove("clickable");
domCache.footerCover.classList.remove("clickable");void 0!==a.result.Artist&&0<a.result.Artist.length&&"-"!==a.result.Artist?(c+=a.result.Artist,f+=a.result.Artist,h+=a.result.Artist+" - ",domCache.footerArtist.innerText=a.result.Artist,domCache.footerArtist.setAttribute("data-name",encodeURI(a.result.Artist)),!0===settings.featAdvsearch&&domCache.footerArtist.classList.add("clickable")):(domCache.footerArtist.innerText="",domCache.footerArtist.setAttribute("data-name",""));void 0!==a.result.Album&&
0<a.result.Album.length&&"-"!==a.result.Album?(c+=" - "+a.result.Album,f+="<br/>"+a.result.Album,domCache.footerAlbum.innerText=a.result.Album,domCache.footerAlbum.setAttribute("data-name",encodeURI(a.result.Album)),domCache.footerAlbum.setAttribute("data-albumartist",encodeURI(a.result[tagAlbumArtist])),!0===settings.featAdvsearch&&domCache.footerAlbum.classList.add("clickable")):(domCache.footerAlbum.innerText="",domCache.footerAlbum.setAttribute("data-name",""));void 0!==a.result.Title&&0<a.result.Title.length?
(h+=a.result.Title,domCache.currentTitle.innerText=a.result.Title,domCache.currentTitle.setAttribute("data-uri",encodeURI(a.result.uri)),domCache.footerTitle.innerText=a.result.Title,domCache.footerTitle.classList.add("clickable"),domCache.footerCover.classList.add("clickable")):(domCache.currentTitle.innerText="",domCache.currentTitle.setAttribute("data-uri",""),domCache.footerTitle.innerText="",domCache.footerTitle.setAttribute("data-name",""),domCache.footerTitle.classList.remove("clickable"),
domCache.footerCover.classList.remove("clickable"));document.title="Ideon: "+h;domCache.footerCover.title=h;void 0!==a.result.uri&&""!==a.result.uri&&-1===a.result.uri.indexOf("://")?domCache.footerTitle.classList.add("clickable"):domCache.footerTitle.classList.remove("clickable");void 0!==a.result.uri?(a.result.Filetype=filetype(a.result.uri),document.getElementById("addCurrentSongToPlaylist").removeAttribute("disabled")):(a.result.Filetype="",document.getElementById("addCurrentSongToPlaylist").setAttribute("disabled",
"disabled"));!0===settings.featStickers&&setVoteSongBtns(a.result.like,a.result.uri);a.result.Fileformat=lastState?fileformat(lastState.audioFormat):"";for(h=0;h<settings.colsPlayback.length;h++){var d=document.getElementById("current"+settings.colsPlayback[h]);if(d&&"Lyrics"===settings.colsPlayback[h])getLyrics(a.result.uri,d.getElementsByTagName("p")[0]);else if(d){var g=a.result[settings.colsPlayback[h]];void 0===g&&(g="");"Duration"===settings.colsPlayback[h]?g=beautifySongDuration(g):"LastModified"===
settings.colsPlayback[h]&&(g=localeDate(g));d.getElementsByTagName("span")[0].innerText=g;d.setAttribute("data-name",encodeURI(g));"Album"===settings.colsPlayback[h]&&null!==a.result[tagAlbumArtist]&&d.setAttribute("data-albumartist",encodeURI(a.result[tagAlbumArtist]))}}document.getElementById("currentBooklet").innerHTML=""===a.result.bookletPath||void 0===a.result.bookletPath||!1===settings.featBrowse?"":'<span class="text-light material-icons">description</span>&nbsp;<a class="text-light" target="_blank" href="'+
subdir+"/browse/music/"+e(a.result.bookletPath)+'">'+t("Download booklet")+"</a>";if(h=document.getElementById("queueTrackId"+a.result.currentSongId))h.getElementsByTagName("td")[2].innerText=a.result.Title;"play"===playstate&&showNotification(a.result.Title,c,f,"success");lastSong=b;lastSongObj=a.result}}function gotoTagList(){appGoto(app.current.app,app.current.tab,app.current.view,"0","-","-","-","")}function volumeStep(a){chVolume("up"===a?settings.volumeStep:0-settings.volumeStep)}
...
function focusTable(a,b){void 0===b&&(b=document.getElementById(app.current.app+(void 0!==app.current.tab?app.current.tab:"")+(void 0!==app.current.view?app.current.view:"")+"List"));if(("Browse"===app.current.app||"Playback"===app.current.app)&&"Database"===app.current.tab&&"List"===app.current.view){if(a=document.getElementsByClassName("card-grid"),0!==a.length){b=a[0];for(var c=0;c<a.length;c++)if(a[c].classList.contains("selected")){b=a[c];break}b.focus()}}else if("Home"===app.current.app){if(a=
document.getElementsByClassName("home-icons"),0!==a.length){b=a[0];for(c=0;c<a.length;c++)if(a[c].classList.contains("selected")){b=a[c];break}b.focus()}}else if(null!==b){c=b.getElementsByClassName("selected");if(void 0===a)0===c.length?(a=b.getElementsByTagName("tbody")[0].rows[0],a.classList.contains("not-clickable")&&(a=b.getElementsByTagName("tbody")[0].rows[1]),a.focus(),a.classList.add("selected")):c[0].focus();else{c&&0<c.length&&c[0].classList.remove("selected");c=b.getElementsByTagName("tbody")[0].rows;
var f=c.length;f<a&&(a=0);f>a&&(c[a].focus(),c[a].classList.add("selected"))}"BrowseFilesystemList"===b.id&&(a=b.getElementsByTagName("tbody")[0],0<a.rows.length&&"parentDir"!==a.rows[0].getAttribute("data-type")&&""!==app.current.search&&(b=b.getElementsByTagName("thead")[0].rows[0].cells.length,c=app.current.search.replace(/\/?([^/]+)$/,""),a=a.insertRow(0),a.setAttribute("data-type","parentDir"),a.setAttribute("tabindex",0),a.setAttribute("data-uri",encodeURI(c)),a.innerHTML='<td colspan="'+b+
'">..</td>'));scrollFocusIntoView()}}function scrollFocusIntoView(){var a=document.activeElement,b=a.getBoundingClientRect().top,c=a.offsetHeight;a=a.parentNode.parentNode.offsetTop;window.innerHeight>window.innerWidth&&(a+=domCache.header.offsetHeight);var f=window.innerHeight-a-domCache.footer.offsetHeight,h=c/2;b<=a+h?window.scrollBy(0,-c):b+c>f-h&&window.scrollBy(0,c)}
...
function setCols(a){var b=document.getElementById(a+"ColsDropdown");b&&(b.firstChild.innerHTML=setColsChecklist(a));b=app.current.sort;"Search"===a&&"Title"===app.apps.Search.sort&&(b=settings.tags.includes("Title")?"Title":!1===settings.featTags?"Filename":"-");if("Playback"!==a){var c="";"QueueCurrent"===a&&(c+='<th data-col="Img"></th>');for(var f=0;f<settings["cols"+a].length;f++){var h=settings["cols"+a][f];c+='<th draggable="true" data-col="'+h+'">';if("Track"===h||"Pos"===h)h="#";c+=t(h);"Search"!==
a||h!==b&&"-"+h!==b||(h=!1,0===app.current.sort.indexOf("-")&&(h=!0),c+='<span class="sort-dir material-icons pull-right">'+(!0===h?"arrow_drop_up":"arrow_drop_down")+"</span>");c+="</th>"}c=!0===settings.featTags?c+('<th data-col="Action"><a data-title-phrase="'+t("Columns")+'" href="#" class="text-secondary align-middle material-icons material-icons-small">settings</a></th>'):c+"<th></th>";document.getElementById(a+"List").getElementsByTagName("tr")[0].innerHTML=c}}
...
h.innerText=c[f].name,h.setAttribute("data-col",c[f].name),b.insertBefore(h,b.lastChild))}}a={table:"cols"+a,cols:[]};b=b.getElementsByTagName("th");for(c=0;c<b.length;c++)f=b[c].getAttribute("data-col"),"Action"!==f&&null!==f&&"Img"!==f&&a.cols.push(f);sendAPI("MYMPD_API_COLS_SAVE",a,getSettings)}
function saveColsPlayback(a){for(var b=document.getElementById(a+"ColsDropdown").firstChild.getElementsByTagName("button"),c=document.getElementById("cardPlaybackTags"),f=0;f<b.length;f++){var h=document.getElementById("current"+b[f].name);!1===b[f].classList.contains("active")?h&&h.remove():h||(h=document.createElement("div"),h.innerHTML="<small>"+t(b[f].name)+": </small><span></span>",h.setAttribute("id","current"+b[f].name),h.setAttribute("data-tag",b[f].name),c.appendChild(h))}a={table:"cols"+
...
function parseEditTimer(a){var b=a.result.playlist;sendAPI("MPD_API_PLAYLIST_LIST_ALL",{searchstr:""},function(h){getAllPlaylists(h,"selectTimerPlaylist",b)});document.getElementById("inputTimerId").value=a.result.timerid;document.getElementById("inputTimerName").value=a.result.name;toggleBtnChk("btnTimerEnabled",a.result.enabled);document.getElementById("selectTimerHour").value=a.result.startHour;document.getElementById("selectTimerMinute").value=a.result.startMinute;document.getElementById("selectTimerAction").value=
...
" - Windows Media Audio";default:return a}}function fileformat(a){return a.bits+t("bits")+" - "+a.sampleRate/1E3+t("kHz")}function scrollToPosY(a){document.body.scrollTop=a;document.documentElement.scrollTop=a;"Playback"===app.current.app&&"List"===app.current.view&&0==a&&(document.getElementById("viewListDatabaseBox").scrollTop=0)}
...
function setPagination(a,b){var c=("Playback"===app.current.app?"Browse":app.current.app)+(void 0===app.current.tab?"":app.current.tab),f=Math.ceil(a/settings.maxElementsPerPage);0===f&&(f=1);for(var h=[document.getElementById(c+"PaginationTop"),document.getElementById(c+"PaginationBottom")],d=0;d<h.length;d++){var g=h[d].children[0],k=h[d].children[1].children[0],l=h[d].children[1].children[1],m=h[d].children[2];k.innerText=app.current.page/settings.maxElementsPerPage+1+" / "+f;if(1<f){k.removeAttribute("disabled");
for(var n="",p=0;p<f;p++)n+='<button data-page="'+p*settings.maxElementsPerPage+'" type="button" class="mr-1 mb-1 btn-sm btn btn-secondary">'+(p+1)+"</button>";l.innerHTML=n;k.classList.remove("nodropdown")}else-1===a?(k.setAttribute("disabled","disabled"),k.innerText=app.current.page/settings.maxElementsPerPage+1):k.setAttribute("disabled","disabled"),k.classList.add("nodropdown");a>app.current.page+settings.maxElementsPerPage||-1===a&&b>=settings.maxElementsPerPage?(m.removeAttribute("disabled"),
h[d].classList.remove("hide"),document.getElementById(c+"ButtonsBottom").classList.remove("hide")):(m.setAttribute("disabled","disabled"),h[d].classList.add("hide"),document.getElementById(c+"ButtonsBottom").classList.add("hide"));0<app.current.page?(g.removeAttribute("disabled"),h[d].classList.remove("hide"),document.getElementById(c+"ButtonsBottom").classList.remove("hide")):g.setAttribute("disabled","disabled")}}function genId(a){return"id"+a.replace(/[^\w-]/g,"")}
...
function gotoPage(a){console.log(app.current.page);document.getElementById("card"+app.current.app).scrollIntoView();"Playback"===app.current.app&&"List"===app.current.view&&(document.getElementById("viewListDatabaseBox").scrollTop=0);switch(a){case "next":app.current.page=parseInt(app.current.page)+parseInt(settings.maxElementsPerPage);break;case "prev":app.current.page=parseInt(app.current.page)-parseInt(settings.maxElementsPerPage);0>app.current.page&&(app.current.page=0);break;default:app.current.page=
a}appGoto(app.current.app,app.current.tab,app.current.view,app.current.page,app.current.filter,app.current.sort,app.current.tag,app.current.search)}function validateFilenameString(a){return""===a?!1:null!==a.match(/^[\w-.]+$/)?!0:!1}function validateFilename(a){if(!1===validateFilenameString(a.value))return a.classList.add("is-invalid"),!1;a.classList.remove("is-invalid");return!0}
...
function validatePlnameEl(a){if(!1===validatePlname(a.value))return a.classList.add("is-invalid"),!1;a.classList.remove("is-invalid");return!0}function validatePlname(a){return""===a?!1:null===a.match(/\/|\r|\n|"|'/)?!0:!1}function validateNotBlank(a){if(""===a.value.replace(/\s/g,""))return a.classList.add("is-invalid"),!1;a.classList.remove("is-invalid");return!0}
...
function validateHost(a){if(null!==a.value.match(/^([\w-.]+)$/))return a.classList.remove("is-invalid"),!0;a.classList.add("is-invalid");return!1}function validateIPAddress(a){if(null!==a.value.match(/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/))return a.classList.remove("is-invalid"),!0;a.classList.add("is-invalid");return!1};
